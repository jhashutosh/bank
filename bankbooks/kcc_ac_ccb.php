...
echo "</title>";
echo "<hr>";
$sql_statement="SELECT * FROM bank_bk_dtl where account_sub_type='kcc' and b_name='H.D.C.C.B' AND status='op'";
//echo "$sql_statement";
$result=dBConnect($sql_statement);
if(pg_NumRows($result)>0){
echo "<table align=center bgcolor=silver width=100%>";
echo "<tr><th colspan=6 bgcolor=#8A2BE2><font color=white>KCC Bank Account Details</font></th>";
echo "<tr bgcolor=GREEN><th>Sl No.<th>A/C No.<th>Bank Name<th>Interest Payable<th>Balance<th>Operation";
		$color=$TCOLOR;
...
if(($row['account_no']))
echo "<a href=\"loan_statement.php?menu=".$row['account_sub_type']."&op=1&id=".$row['link_tb']."&account_no=".$row['account_no']."\" onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=100,left=150, width=1200,height=580'); return false;\">Statement</a>";
