...
$dr_cr='coalesce(dr_amt,0)-coalesce(cr_amt,0)';
if($gl== 40000 || $gl== 50000)
$dr_cr='coalesce(cr_amt,0)-coalesce(dr_amt,0)';
if($gl==20000)
$dr_cr='coalesce(gl_mas_bl_prv,0)+coalesce(dr_amt,0)-coalesce(cr_amt,0)';
if($gl==10000)
$dr_cr='coalesce(gl_mas_bl_prv,0)+coalesce(cr_amt,0)-coalesce(dr_amt,0)';
...
<caption>
	...
	<!--<b>CURRENT YEAR AMOUNT(Rs):</b><?php echo amount2Rs($row['curr_bal']);?><br/>
	<b>PREVIOUS YEAR AMOUNT(Rs):</b><?php echo amount2Rs($row['prev_bal']);?>style="height:410px;overflow-y:auto;width:100%"-->
</caption>
<tr>
	<th width='10%'>CODE</th>
...
<tr  class='tr0'>
	...
	<td width='35%'><?php echo $row['head'];?></td>
	<td width='30%'><?php echo amount2Rs($row['curr_bal']);?></td>
...
<tr><td colspan='4'>
	...
	<table class='subTable'  cellspacing=0 cellpadding=0>		
		<?php }if($row['od']==1){?>
			<tr  class='tr1'>
				<td width='10%'><?php echo $row['code'];?></td>
				<td width='35%'><?php echo ucwords($row['head']);?></td>
				<td width='30%'><?php echo amount2Rs($row['curr_bal']);?></td>
				<td width='25%'><?php echo amount2Rs($row['prev_bal']);?></td>
			</tr>
		<?php }if($row['od']==2){?>
