...
<caption>
	...
	<b>PREVIOUS YEAR AMOUNT(Rs):</b><?echo amount2Rs($row['prev_bal']);?>-->
	<?echo "<font size=+3>$SYSTEM_TITLE</font> <br><font size=+1><b>Report Date:".date('d.m.Y')."::".getPrintTime()."</b></font><hr>";?>
</caption>
<tr>
	<th width='10%'>CODE</th>
...
<tr  class='tr0'>
	...
	<td width='40%'><?echo $row['head'];?></td>
	<td width='25%'><?echo amount2Rs($row['curr_bal']);?></td>
...
<tr><td colspan='4'>
	...
	<table class='subTable'>		
		<?}if($row['od']==1){?>
			<tr  class='tr1'>
				<td width='10%'><?echo $row['code'];?></td>
				<td width='40%'><?echo ucwords($row['head']);?></td>
				<td width='25%'><?echo amount2Rs($row['curr_bal']);?></td>
				<td width='25%'><?echo amount2Rs($row['prev_bal']);?></td>
			</tr>
		<?}if($row['od']==2){?>
