...
$sql_statement="SELECT account_no FROM shg_account WHERE shg_no='$group_no' AND account_type='jgl' AND status='op' AND account_opening_date <=current_date";  
echo $sql_statement;
...
for($j=0;$j<pg_NumRows($result);$j++){
  ...
  echo "<option value=\"".$row['account_no']."\">".$row['account_no'];
  echo "</option>";}
echo "</select>";  
