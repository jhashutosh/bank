...
echo "<FORM NAME=\"form1\" method=\"POST\" action=\"ofl_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify1();\">";
echo "<table bgcolor=\"#FAEBD7\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FF00FF\" colspan=\"6\"><font size=+2>Own Fund Loan Issuing Form[$account_no]</font>";
...
if($_REQUEST['op']=='v'){
	...
	echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"ofl_loan_issue_ef.php?menu=$menu&op=i\">";
echo "<table bgcolor=\"#FAEBD7\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FF00FF\" colspan=\"6\"><font size=+2>Own Fund Loan Issuing Form For verify[$account_no]</font>";
...
$gl_code=getGlCode4mCustomerAccount($account_no,$action_date);
	...
	if(empty($fy1)){
		echo "<h1><center><b>You Can't Insert any value Into database Because Financial Year already Locked by administrator !!!!!!!!!</h1>";
	 	...
	 	$t_id=getTranId();
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,period,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status) VALUES ('$loan_sl_no','ofl','$id','$account_no', '$fy1','$action_date','$repay_date',$period,$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time) VALUES ('$t_id','ofl','$action_date','$fy1','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
//gl_ledger_dtl ===== Dr.
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$issue_amount,'Dr','House Loan Issue')";
//gl_ledger_dtl ===== Cr.
	//For Cash 
		if(empty($account_sb)){
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$issue_amount,'Cr','House Loan Issue')";
		}
	//For Transfer to SB A/C.
		else{
		$gl_code=getGlCode4mCustomerAccount($account_sb,$action_date);
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars,account_no) VALUES('$t_id','$gl_code',$issue_amount,'Cr','House Loan Issue','$account_sb')";
		}
		//echo $sql_statement;
