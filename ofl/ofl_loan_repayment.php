...
$sql_statement="SELECT getLoanInterest('$account_no','$action_date','ofl') as int";
//echo $sql_statement;
...
if(pg_NumRows($result)>0){
	$color="GREEN";
	echo "<tr><th bgcolor=YELLOW colspan=\"9\">Own Fund Loan Details of [$account_no] as on ".$action_date."</th>";
	echo "<tr>";
	...
	echo "<td align=Center bgcolor=$color><a href=\"ofl_loan_repayment.php?menu=$menu&ln_sl=$loan_sl_no&op=r&action_date=$action_date\">Repay</td>";

	}
}
else{
 echo "<h1><center>Your dont have any Own Fund Loan!!!!!!!!!!!</h1></center>";
    }
  }
else{
 echo "<br><h1><font color=RED><center>Your dont have any Own Fund Loan for Payment!!!!!!!!!!!</h1></center>";
    }
}
//==================================FOR REPAY FORM=========================================
if($op=='r'){
...
echo "<tr><th bgcolor=\"#808000\" colspan=\"4\"><font size=+2>Own Fund Loan Repayment Form of [$account_no]</font>";
echo "<FORM NAME=\"pForm\" method=\"POST\" action=\"ofl_loan_repayment.php?menu=$menu&op=i&ln_sl=$ln_sl\" onSubmit=\"return checkKccReturn(this.form);\">";
...
if(empty($fy1)){
	...
	//For GL ENTRY-------------------------------------
	$sql_statement.=";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','ofl','$action_date','$fy1','$remarks','$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$amount,'Dr','Own Fund Repay')";
	if($r_i_od>0){
