...
union all
select a.gl_mas_code, initcap(b.gl_mas_desc) as gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(debit-credit) as bal from mas_gl_tran as a , gl_master_vw1 as b WHERE action_date<='$start_date'  AND a.gl_mas_code= b.gl_mas_code and (bs_pl='Z' or bs_pl='O') group by a.gl_mas_code, b.gl_mas_desc 
UNION ALL
...
echo "<th colspan=\"2\" bgcolor=\"Yellow\">Profit & Loss Appropriation A/C as on $f_end_dt<tr>";
...
union all
SELECT a.gl_mas_code, SUM(debit) as debit,SUM(credit) as credit,bs_pl FROM mas_gl_tran  a ,gl_master_vw b where a.gl_mas_code::text='12301' and action_date<'$start_date' AND a.gl_mas_code=b.gl_mas_code GROUP BY a.gl_mas_code ,bs_pl) as foo order by gl_mas_code DESC)
