...
echo "<hr>";
$sql_statement="
select gl_mas_code,gl_mas_desc,debit,credit,bal
from
(
select gl_mas_code,gl_mas_desc,debit,credit,
case  when gl_mas_code='12302' and  bal<0 then 0  else bal end as bal
from
(

select a.gl_mas_code, initcap(b.gl_mas_desc) as gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(debit-credit) as bal from mas_gl_tran as a , gl_master_vw as b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code= b.gl_mas_code and bs_pl='E' group by a.gl_mas_code, b.gl_mas_desc 

UNION ALL
SELECT '12302' as gl_mas_code, 'Net Profit' as gl_mas_desc,SUM(debit) as debit,SUM(credit) as cedit,SUM(credit-debit) as bal 
FROM 
(SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='I' OR bs_pl='P' OR bs_pl='E') 
as foo

) as a 

) as a where bal<>0 order by gl_mas_code";
//echo $sql_statement;
...
$sql_statement="
select gl_mas_code, gl_mas_desc,debit, credit,
case  when gl_mas_code='12302' and  bal<0 then ABS(bal)  else bal end as bal
from
(
...
(SELECT a.gl_mas_code, b.gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(credit-debit) as bal from mas_gl_tran as a , gl_master_vw as b where action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code= b.gl_mas_code and bs_pl='I' group by a.gl_mas_code, b.gl_mas_desc)

UNION ALL
(SELECT '39000' as gl_mas_code, 'Gross prifit' as gl_mas_desc,SUM(debit) as debit,SUM(credit) as credit,SUM(credit-debit) as bal FROM (SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='P') as foo order by gl_mas_code)

UNION ALL
(SELECT '12302' as gl_mas_code, 'Net Loss' as gl_mas_desc,SUM(debit) as debit,SUM(credit) as credit,SUM(credit-debit) as bal 
FROM 
(SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='I' OR bs_pl='P' OR bs_pl='E') 
as foo)
