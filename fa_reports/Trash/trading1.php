...
echo "<hr>";
$sql_statement="
SELECT a.gl_mas_code, initcap(b.gl_mas_desc) as gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(debit-credit) as bal from mas_gl_tran as a , gl_master_vw as b where action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code= b.gl_mas_code and bs_pl='P' group by a.gl_mas_code, b.gl_mas_desc 
UNION ALL
SELECT '39000' as gl_mas_code, 'Gross Profit' as gl_mas_desc,SUM(debit) as debit,SUM(credit) as cedit,SUM(credit-debit) as bal FROM (SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='P'
) as foo order by gl_mas_code";
//echo $sql_statement;
$result=dBConnect($sql_statement);
echo "<table width=\"100%\" bgcolor=\"\"><tr>";
echo "<th colspan=\"2\" bgcolor=\"Yellow\">Trading A/C as on $start_date<tr>";
...
$sql_statement="select a.gl_mas_code, initcap(b.gl_mas_desc) as gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(credit-debit) as bal from mas_gl_tran as a , gl_master_vw as b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code= b.gl_mas_code and bs_pl='S' group by a.gl_mas_code, b.gl_mas_desc order by a.gl_mas_code
union all
select a.gl_mas_code, initcap(b.gl_mas_desc) as gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(credit-debit) as bal from mas_gl_tran as a , gl_master_vw as b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code= b.gl_mas_code and a.gl_mas_code='34100' group by a.gl_mas_code, b.gl_mas_desc order by a.gl_mas_code
union all
select a.gl_mas_code, initcap(b.gl_mas_desc) as gl_mas_desc, sum(debit) as debit, sum(credit) as credit, sum(credit-debit) as bal from mas_gl_tran as a , gl_master_vw as b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code= b.gl_mas_code and a.gl_mas_code='41195' group by a.gl_mas_code, b.gl_mas_desc order by a.gl_mas_code
";
