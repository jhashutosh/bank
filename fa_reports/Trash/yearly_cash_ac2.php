...
$opbal=$row1['opb'];
$sql_statement= "select gl_sub_header_code,gl_sub_header_desc, sum(b.cnt) as cnt, sum(b.received) as received, sum(b.transfer) as transfer from gl_sub_header_vw as a, ( 
...
UNION ALL 
SELECT 'R' as typ, a.gl_mas_code, count(*) as cnt, 0.00 as received, sum(credit-debit) as transfer, 0.00 as Payment from mas_gl_tran as a where debit-credit<0 and  action_date>='$start_date' AND action_date<='$end_date' AND tran_id  IN (SELECT tran_id FROM mas_gl_tran where gl_mas_code LIKE '282%' OR gl_mas_code LIKE '283%' AND tran_id NOT IN (SELECT tran_id FROM mas_gl_tran where action_date>='$start_date' AND action_date<='$end_date' GROUP BY tran_id HAVING COUNT(*)<2)) group by a.gl_mas_code 
...
$color=$TCOLOR;
	...
	$sql_statement1="SELECT b.gl_mas_code,initcap(gl_mas_desc) AS gl_mas_desc, sum(b.received) as balance from gl_sub_header_vw as a, ( 
select 'R' as typ, a.gl_mas_code, count(*) as cnt, sum(credit-debit) as received, 0.00 as transfer, 0.00 as Payment from mas_gl_tran as a where a.gl_mas_code not  like '28101' and tran_id in (select tran_id from mas_gl_tran where gl_mas_code like '28101' and debit-credit>0 and action_date>='$start_date' AND action_date<='$end_date') group by a.gl_mas_code
...
$sql_statement= "select gl_sub_header_code,gl_sub_header_desc, sum(b.cnt) as cnt, sum(b.payment) as payment, sum(b.transfer) as transfer from gl_sub_header_vw as a, ( 
...
for($j=0; $j<pg_NumRows($result); $j++) {
	...
	$sql_statement1="select b.gl_mas_code,initcap(gl_mas_desc) AS gl_mas_desc, sum(b.payment) as balance, sum(b.transfer) as transfer from gl_sub_header_vw as a, ( 
select 'P' as typ,a.gl_mas_code, count(*) as cnt, 0.00 as received, 0.00 as transfer, sum(debit-credit) as Payment from mas_gl_tran as a where a.gl_mas_code not like '28101' and tran_id in (select tran_id from mas_gl_tran where gl_mas_code like '28101' and debit-credit<0 and action_date>='$start_date' AND action_date<='$end_date') group by a.gl_mas_code HAVING sum(debit-credit)>0 
) as b where  a.gl_mas_code=b.gl_mas_code AND gl_sub_header_code='$gl_code'
GROUP BY  b.gl_mas_code,gl_mas_desc
ORDER BY b.gl_mas_code";
