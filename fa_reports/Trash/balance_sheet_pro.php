...
echo "<hr>";
$sql_statement="
SELECT gl_mas_code,gl_mas_desc,
p_balance,
c_balance
from
(
...
SELECT gl_mas_code,gl_mas_desc,
case  when gl_mas_code='12304' and  p_balance<0 then 0  else  p_balance end as p_balance,
case  when gl_mas_code='12304' and  c_balance<0 then 0  else  c_balance end as c_balance
from
(

SELECT gl_mas_code,gl_mas_desc,SUM(p_balance) as p_balance,SUM(c_balance) as c_balance FROM (
...
--UNION ALL
--SELECT '12302' as gl_mas_code, 'Profit & Loss Balance As Per Current Year' as gl_mas_desc,SUM(credit-debit) as p_balance,0 as c_balance FROM (SELECT 
--a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE (action_date BETWEEN date('$f_start_dt')
...
) AS fo GROUP BY gl_mas_code,gl_mas_desc HAVING gl_mas_code not in('12302','12301') 

) as a 

) as a where p_balance<>0 or c_balance<>0 order by gl_mas_code";
//echo $sql_statement;
...
SELECT gl_mas_code,gl_mas_desc,
case  when gl_mas_code='12304' and  p_balance<0 then ABS(p_balance)  else p_balance end as p_balance,
case  when gl_mas_code='12304' and  c_balance<0 then ABS(c_balance)  else c_balance end as c_balance
from
(
...
UNION ALL
SELECT '12304' as gl_mas_code, 'Accumulated Loss' as gl_mas_desc,0 as p_balance,SUM(credit-debit) as c_balance 
FROM 
(SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE action_date BETWEEN '$f_start_dt' AND '$start_date' AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='I' OR bs_pl='P' OR bs_pl='E' OR bs_pl='Z'
union all
SELECT a.gl_mas_code, SUM(debit) as debit,SUM(credit) as credit,bs_pl FROM mas_gl_tran  a ,gl_master_vw b where a.gl_mas_code::text='12301' and action_date<'$start_date' AND a.gl_mas_code=b.gl_mas_code GROUP BY a.gl_mas_code ,bs_pl
...
UNION ALL
SELECT '12304' as gl_mas_code, 'Accumulated Loss' as gl_mas_desc,SUM(credit-debit) as p_balance,0 as c_balance 
FROM 
(--SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE action_date BETWEEN '$f_start_dt' AND 
--'$start_date' AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='I' OR bs_pl='P' OR bs_pl='E' OR bs_pl='Z'
--union all
...
) AS foo GROUP BY gl_mas_code,gl_mas_desc 
