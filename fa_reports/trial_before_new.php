...
SELECT gl_mas_code,0 as op_dr,CASE WHEN CAST(gl_mas_code AS INT)<30000 THEN SUM(gl_mas_bl_prv) ELSE 0 END as op_cr,SUM(dr_amt) as dr,SUM(cr_amt) as cr,0 as dr_bal,CASE WHEN CAST(gl_mas_code AS INT)<30000 THEN  SUM(cr_amt+gl_mas_bl_prv-dr_amt) ELSE SUM(cr_amt-dr_amt) END as cr_bal FROM sch_bs_sh_gm WHERE fn_nm IN ('bs_sch','P/L') AND (CAST(gl_mas_code AS INT) BETWEEN 10000 AND 19999 OR CAST(gl_mas_code AS INT) BETWEEN 40000 AND 59999) AND CAST(gl_mas_code AS INT)NOT IN(12302) GROUP BY gl_mas_code 
UNION ALL 
...
UNION ALL 
SELECT gl_mas_code,CASE WHEN CAST(gl_mas_code AS INT)<30000 THEN SUM(gl_mas_bl_prv) ELSE 0 END,0,SUM(dr_amt) as dr,SUM(cr_amt) as cr,CASE WHEN CAST(gl_mas_code AS INT)<30000 THEN SUM(dr_amt-cr_amt+gl_mas_bl_prv) ELSE SUM(dr_amt-cr_amt) END as dr_bal,0 FROM sch_bs_sh_gm WHERE fn_nm IN ('bs_sch','P/L') AND (CAST(gl_mas_code AS INT) BETWEEN 20000 AND 39999 OR CAST(gl_mas_code AS INT) BETWEEN 60000 AND 79999) GROUP BY gl_mas_code 
...
echo "<tr>";
echo "<th bgcolor=$color colspan=\"8\"><font size='2'>TRIAL BALANCE(Before) AS ON : $start_date </font></th></tr>";
...
echo "<td bgcolor=$color colspan=\"1\"  width=\"39%\" style=\"height:25px;\">".$row['gl_mas_desc']."</td>";
