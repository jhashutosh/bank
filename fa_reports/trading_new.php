...
//---------------------------------------------------------------------------------------------
echo "<hr>";
$sql_statement="select * from (select s.gl_sub_header_code, INITCAP(gl_sub_header_desc) as gl_sub_header_desc, sum(dr_amt) dr, sum(cr_amt) cr, sum(dr_amt-cr_amt) bl,sum(gl_mas_bl_prv) as bl_p from sch_bs_sh_gm s, gl_sub_header g where g.gl_sub_header_code=s.gl_sub_header_code group by s.gl_sub_header_code, gl_sub_header_desc order by s.gl_sub_header_code) s where (bl<>0 OR  bl_p<>0) and CAST(gl_sub_header_code as BIGINT) BETWEEN 30000 AND 39999
UNION ALL
SELECT gl_sub_header_code, gl_mas_desc,SUM(dr) as dr,SUM(cr)as cr,SUM(bl) as bl,SUM(bl_p) as bl_p FROM (
select '00000' as gl_sub_header_code, 'Gross Profit' as gl_mas_desc, 0 as dr,0 as cr, gl_mas_bl as bl,coalesce(gl_mas_bl_prv,0) as bl_p from sch_bs_sh_gm where CAST(gl_mas_code as BIGINT)=1 AND coalesce(gl_mas_bl,0)>0
UNION ALL
select '00000' as gl_sub_header_code, 'Gross Profit' as gl_mas_desc, 0 as dr,0 as cr, 0 as bl,coalesce(gl_mas_bl_prv,0) as bl_p from sch_bs_sh_gm where CAST(gl_mas_code as BIGINT)=1 AND coalesce(gl_mas_bl_prv,0)>0) as foo group by gl_sub_header_code, gl_mas_desc";
//echo $sql_statement;
$result=dBConnect($sql_statement);
echo "<table width=\"100%\" border=\"1\" cellspacing=\"0\" bordercolor=\"BLACK\" bordercolorlight=\"BLACK\" bordercolordark=\"BLACK\"><tr>";
echo "<th colspan=\"2\" bgcolor=\"Yellow\"><font size=\"+3\">Trading A/C as on $start_date</font><tr>";
...
for($j=0; $j<pg_NumRows($result); $j++){
	...
	echo "<tr>";
if ($row['gl_sub_header_code']!='000000')
...
$sql_statement="select * from (select s.gl_sub_header_code, INITCAP(gl_sub_header_desc) as gl_sub_header_desc, sum(dr_amt) dr, sum(cr_amt) cr, sum(cr_amt-dr_amt) bl,sum(gl_mas_bl_prv) as bl_p from sch_bs_sh_gm s, gl_sub_header g where g.gl_sub_header_code=s.gl_sub_header_code group by s.gl_sub_header_code, gl_sub_header_desc order by s.gl_sub_header_code) s where (bl<>0 OR  bl_p<>0) and CAST(gl_sub_header_code as BIGINT) BETWEEN 40000 AND 49999
UNION ALL
SELECT gl_sub_header_code, gl_mas_desc,SUM(dr) as dr,SUM(cr)as cr,SUM(bl) as bl,SUM(bl_p) as bl_p FROM (
select '00000' as gl_sub_header_code, 'Gross Loss' as gl_mas_desc, 0 as dr,0 as cr, abs(coalesce(gl_mas_bl,0))  as bl,abs(coalesce(gl_mas_bl_prv,0)) as bl_p from sch_bs_sh_gm  where CAST(gl_mas_code as BIGINT)=1 AND coalesce(gl_mas_bl,0)<0
UNION ALL
select '00000' as gl_sub_header_code, 'Gross Loss' as gl_mas_desc, 0 as dr,0 as cr, 0 as bl,abs(coalesce(gl_mas_bl_prv,0)) as bl_p from sch_bs_sh_gm  where CAST(gl_mas_code as BIGINT)=1 AND coalesce(gl_mas_bl_prv,0)<0) as foo group by gl_sub_header_code, gl_mas_desc";
...
$color="AQUA";
echo "</td></tr>";
echo "<tr><td align=left><table width=\"100%\" cellspacing=\"0\"  border=\"1\">";
echo "<tr bgcolor=$color>";
echo "<th colspan=\"2\" width=\"\" >Total</th>";
echo "<th colspan=\"1\" width=\"19%\" align=\"RIGHT\">".amount2Rs($Lbal)."</th>";
echo "<th  colspan=\"1\" width=\"9%\" align=\"RIGHT\">".amount2Rs($P_Lbal)."</th>";
echo "</tr>";
echo "</table >";
echo "<td align=left><table width=\"100%\"  cellspacing=\"0\" border=\"1\">";
echo "<tr bgcolor=$color>";
echo "<th colspan=\"2\" width=\"\">Total</th>";
echo "<th  colspan=\"1\" width=\"19%\" align=\"RIGHT\">".amount2Rs($Abal)."</th>";
echo "<th  colspan=\"1\" width=\"9%\" align=\"RIGHT\">".amount2Rs($P_Abal)."</th>";
echo "</tr></table>";
echo "</td></tr></table>";
echo "</body>";
echo "</html>";
