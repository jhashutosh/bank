...
SELECT gl_mas_code,CASE WHEN CAST(gl_mas_code AS INT)<30000 THEN SUM(gl_mas_bl_prv) ELSE 0 END as op_dr,0,SUM(dr_amt) as dr,SUM(cr_amt) as cr,CASE WHEN CAST(gl_mas_code AS INT)<30000 THEN SUM(dr_amt-cr_amt+gl_mas_bl_prv) ELSE SUM(dr_amt-cr_amt) END as dr_bal,0 FROM sch_bs_sh_gm WHERE fn_nm IN ('bs_sch','P/L') AND (CAST(gl_mas_code AS INT) BETWEEN 20000 AND 39999 OR CAST(gl_mas_code AS INT) BETWEEN 60000 AND 79999) GROUP BY gl_mas_code 
...
echo "<tr>";
echo "<th bgcolor=$color colspan=\"6\"><font size='2'>Cash Cum Trial Balance(Before) -Dr as on : $start_date </font></th></tr>";
echo "<tr>";
echo "<th bgcolor=$color width=\"8%\" colspan=\"1\" width=\"8%\" >Code</th>";
echo "<th bgcolor=$color   width=\"37%\" colspan=\"1\" width=\"37%\">Account Name</th>";
echo"<th bgcolor=$color width=\"7%\" colspan=\"1\" width=\"16%\">Opening Balance Dr.</th>";
