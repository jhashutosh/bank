...
echo "<form name=\"f1\" action=\"trial_balance_cr.php\" method=\"POST\" onsubmit=\"return check();\">";
...
from(
SELECT case  when gl_mas_code='12301' and  cl_bal_debit<>0 then null else gl_mas_code end as gl_mas_code,
case  when gl_mas_code='12301' and  cl_bal_debit<>0 then null else gl_mas_desc end as gl_mas_desc,
op_bal,
curr_debit, curr_credit,
cl_bal_debit,
cl_bal_credit 
...
) as a, gl_master_vw as b WHERE a.gl_mas_code=b.gl_mas_code  AND (bs_pl='S' OR bs_pl='I' OR bs_pl='L')  GROUP BY a.gl_mas_code, b.gl_mas_desc ORDER BY a.gl_mas_code
...
echo "<tr><td bgcolor=\"green\" colspan=\"7\" align=\"center\"><font color=\"white\"><b>Cash Cum Trial Balance[Liabilities And Income]</b></font></td>";
...
for($j=0; $j<pg_NumRows($result); $j++){
	...
	//$dr_bal=$row['cl_bal_debit'];
	//$cr_bal=$row['cl_bal_credit']
...
//	echo "<td align=right bgcolor=$color>".amount2Rs($row['cl_bal_debit'])."</td>";
	echo "<td align=right bgcolor=$color>".amount2Rs($row['cl_bal_credit']-$row['cl_bal_debit'])."</td>";
	$vop+=$row['op_bal'];
	$vdebit+=$row['curr_debit'];
	$vcredit+=$row['curr_credit'];
//	$cldebit+=$row['cl_bal_debit'];
	$clcredit+=$row['cl_bal_credit']-$row['cl_bal_debit'];
...
//if(pg_NumRows($result)>0){
	echo "<tr>";
	echo "<th align=left bgcolor=$color>$nbsp;</td>";
