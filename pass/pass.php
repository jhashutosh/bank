...
$acdt=date('d/m/Y');
$staff_code=verifyAutho();
$operator_code=$_REQUEST['tsid'];
...
$did=$_REQUEST['did'];
if($op=='f'){
$sql_string="select * from pass_temp where tran_date='$tran_date'";
$result_st=dBConnect($sql_string);
for($j=0;$j<pg_NumRows($result_st);$j++){
$string=pg_fetch_array($result_st,$j);
//echo $string['tran_id'];
$s=$s.$string['tran_id'].','.$string['account_no'].','.$string['amount'].','.$string['tran_date'].','.$string['id_remarks'].','.$string['operator_code'].','.$string['action_date'].','.$string['with_rem'].','.$string['chq_no'].','.$string['chq_dt'].','.$staff_code.'|';
}
$func="select pass_auth_inter('$s')";
echo $func;
$func_res=dBConnect($func);
}
if($del=='y'){
$del="delete from pass_temp where entry_time_pas='$did'";
...
$sql="select * from passing_withdraw_status where tran_dt='$tran_date' and (passing_status=0 OR passing_status=-1)";
$res=dBConnect($sql);
...
$bal_sql="select get_acc_bal_latest('$account_no','$tran_date')";
$bal_res=dBConnect($bal_sql);
//echo $bal_sql;
$bal=pg_fetch_array($bal_res,0);
$balance=$bal['get_acc_bal_latest'];
$sq="select * from pass_temp where entry_time_pas='".$row['entry_time']."'";
//echo $sq;
$r=dBConnect($sq);

$t=pg_fetch_array($r);
if($row['entry_time']==$t['entry_time_pas'])
{
//echo $t['remarks'];
$rem_sql="select * from passing_remarks where id='".$t['id_remarks']."'";
//echo $rem_sql;
...
echo"<input type='hidden' name='tsid' value='".$row['e_staff_id']."'>";
echo"<td bgcolor=$color width='20%'>";
makeSelectFromDbwithId('passing_remarks','id','remarks','remark');
echo"</td>";
