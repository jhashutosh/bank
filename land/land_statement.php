...
if($flag==1){
$sql_statement="SELECT * FROM land_info WHERE customer_id='$account_no' AND status<>'s' AND land_flag=1";
//echo $sql_statement;
$result=dBConnect($sql_statement);
if(pg_NumRows($result)==0) {
...
echo "<td align=center bgcolor=$color><a href=\"land_ledger_ef.php?menu=ln&status=cust\">New </a>&nbsp||&nbsp<a href=\"land_ledger_ef.php?menu=ln&op=up&status=cust&land_id=".$row['land_id']."\">Alter </a>&nbsp||&nbsp<a href=\"land_statement.php?menu=ln&op=s&land_id=".$row['land_id']."\">Less </a></td>";
   }
