...
$sql_statement="SELECT getLoanInterest('$account_no','$action_date','ccl') as int";
//echo $sql_statement;
...
function f2(str,ac_no,menu){
	...
	if(str=='ch')
	{
	getcclhintrepay(str,ac_no,menu);
...
if(pg_NumRows($result)>0){
	$color="GREEN";
	echo "<tr><th bgcolor=YELLOW colspan=\"9\">Cash Credit Loan Details of [$account_no] as on ".$action_date."</th>";
	echo "<tr>";
	...
	echo "<td align=Center bgcolor=$color><a href=\"ccl_loan_repayment.php?menu=$menu&ln_sl=$loan_sl_no&op=r&action_date=$action_date\">Repay</td>";

	}
}
else{
 echo "<h1><center>Your dont have any Cash Credit Loan!!!!!!!!!!!</h1></center>";
    }
  }
else{
 echo "<br><h1><font color=RED><center>Your dont have any Cash Credit Loan for Payment!!!!!!!!!!!</h1></center>";
    }
}
//==================================FOR REPAY FORM=========================================
if($op=='r'){
...
//$gl_status=pg_result($result,'status');
echo "<table bgcolor=\"#F5F5DC\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#808000\" colspan=\"4\"><font size=+2>Cash Credit Loan Repayment Form of [$account_no]</font>";
echo "<FORM NAME=\"pForm\" method=\"POST\" action=\"ccl_loan_repayment.php?menu=$menu&op=i&ln_sl=$ln_sl\" onSubmit=\"return cr_chk();\">";
...
echo "<tr><td>Total:<td>Rs.&nbsp;<INPUT NAME=\"total_r\" TYPE=\"TEXT\" VALUE=\"".($o+$p+$d)."\" READONLY $HIGHLIGHT size=\"10\" >";
echo "<td>Total:<td>Rs.&nbsp;<INPUT NAME=\"total_d\" id=\"total_d\" TYPE=\"TEXT\" VALUE=\"".($principal+$odue_i+$due_i)."\" $HIGHLIGHT READONLY size=\"10\" id=\"o\" READONLY>";
...
echo"<tr><td align=\"right\">Loan Repay Method</td><td width=\"2%\"  align=\"center\">:</td><td align=\"left\" colspan=\"2\"><select name='issue' onchange=\"f2(this.value,'$account_no','$menu');\"><option value=\"ca\">Cash</option></select></td></tr>";
...
*/
	...
	//For GL ENTRY-------------------------------------
	if($issue=='ca'){
	$sql_statement.=";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','ccl','$action_date','$fy','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
         $sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$amount,'Dr','ccl repay')";}
if($issue=='ch'){

$sql_statement.=";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','ccl','$action_date','$fy','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$chq_no','$action_date')";
