...
{
	if(document.f1.action_date.value.length==0){
	alert("Please enter Issuing Date!!!!!");
	document.f1.action_date.value="";
	document.f1.action_date.focus();
	return false;
	}
	if(document.f1.repay_date.value.length==0||!(IsPNumeric(document.f1.repay_date.value))){
	alert("Please enter Repay Date!!!!!");
	document.f1.repay_date.value="";
	document.f1.repay_date.focus();
	return false;
	}
	if(document.f1.issued_amount.value.length==0||!(IsPNumeric(document.f1.issued_amount.value))){
	alert("Please enter Issuing Amount!!!!!");
	document.f1.issued_amount.value="";
	document.f1.issued_amount.focus();
	return false;
	}
	if(document.f1.bal_p.value.length==0||!(IsPNumeric(document.f1.bal_p.value))){
	alert("Please enter Balance Principal!!!!!");
	document.f1.bal_p.value="";
	document.f1.bal_p.focus();
	return false;
	}
	if(document.f1.repay_date.value.length==0||!(IsPNumeric(document.f1.repay_date.value))){
	...
	var issue_amount=parseFloat(document.f1.issued_amount.value);
	var balance_prin=parseFloat(document.f1.bal_p.value);
	 	if(balance_prin>issue_amount)
		{
...
//general ledger posting
	//gl_ledger_hrd
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','bdl','$curr_date','$fy','$remarks', '$staff_id', CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
	//gl_ledger_dtl	
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$balance_p,'Dr','opening BDL')";
	...
	if(pg_affected_rows($result)<1){
		...
		echo "<font size=+2><b>Transaction Id:$t_id<br>";
...
echo "<tr><th bgcolor=\"#006400\" colspan=\"4\"><font size=+2>Bond Loan Opening Form of [$account_no]</font>";
echo "<FORM NAME=\"f1\" method=\"POST\" action=\"bdl_loan_balance_ef.php?menu=$menu&op=i\" onSubmit=\"return beforeVarify();\">";
...
if(empty($loan_sl_no)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=bdl&ls=o','win2')\">";
}
else{
...
echo "<td>Balance Principal Amount:<td>Rs.&nbsp;<INPUT NAME=\"bal_p\" TYPE=\"TEXT\" VALUE=\"\" id=\"bal_p\" $HIGHLIGHT size=\"10\">";
