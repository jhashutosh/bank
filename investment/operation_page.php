...
$fy=$_SESSION['fy'];
	...
	echo "<h1><center><b>You Can't Insert any value Into database Because Financial Year already Locked by administrator !!!!!!!!!";
		...
		$sql_statement="INSERT INTO gl_ledger_hrd(tran_id,type,action_date,fy,remarks, cheque_no,cheque_dt,operator_code,entry_time) VALUES ('$t_id','c".trim($menu)."','$action_date','$fy', '$remarks','$ch_no','$ch_dt','$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
		if($op=='d'){
			...
			$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$bank_account_no','$bk_gl_code',$deposit,'Cr','trf to $account_no')";
			}
		}
		if($op=='w'){
			if($particulars=='c'){
			$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no',$withdrawal,'Dr','charge')";
			}
			else{
	...
	echo "<input type=button onclick=\"myRefresh('bk_investment_rd.php?menu=rd')\" value=\"Return\"> </table>";
	}
	}
