...
$sql_statement="INSERT INTO loan_ledger_hrd(loan_serial_no,loan_type,customer_id,fy,account_no,issue_date,repay_date,period,int_due_rate, int_overdue_rate,payment_term,status,gl_code,gl_status,staff_id,entry_time,loan_compt_status) VALUES ('$loan_sl_no','ser','$id','$fy1','$account_no','$action_date', '$repay_date','$repay_period',$due_int,$overdue_int,'$installment_mode','op','$gl_code_loan','$gl_status','$staff_id',CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'in')";
...
$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','$menu','$curr_date','$fy1','$remarks', '$staff_id', CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";

if($b_d_principal>0)
$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$b_d_principal,'Dr','opening SER')";
if($b_od_prin>0)
{
	if($loan_status<>'o')
	{
		$gl_code_loan=$gl_code_loan+1;
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$b_od_prin,'Dr','opening SER')";
	}
	else
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$b_od_prin,'Dr','opening SER')";
}
...
echo "<tr><th bgcolor=\"#006400\" colspan=\"4\"><font size=+2>Service Loan Issue Form of [$account_no]</font>";
echo "<FORM NAME=\"f1\" method=\"POST\" action=\"service_loan_balance_ef.php?menu=$menu&op=v&account_no=$account_no&type=$type&repay_period=$repay_period&d_int_r=$due_int&od_int_r=$overdue_int&issued_amount=$issued_amount\">";
...
if(empty($loan_sl_no)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=$menu&ls=o','win2')\">";
}
else{
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=$menu','win2')\" DISABLED>";
}
*/
...
echo "<FORM NAME=\"f1\" method=\"POST\" action=\"service_loan_balance_ef.php?menu=$menu&op=i&account_no=$account_no&type=$type&repay_date=$repay_period&action_date=$action_date&d_int_r=$due_int&od_int_r=$overdue_int&issued_amount=$issued_amount&loan_status=$loan_status&repayment_mode=$comp_method&installment_mode=$installment_mode\">";
echo "<table bgcolor=\"black\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"5\"><font size=+2>Service Loan Issue Form of [$account_no]</font></tr>";
...
echo"</tr>";
//echo "";
echo "<tr bgcolor=\"$colour\"><td colspan=\"2\">Balance Due Principal:<td>Rs.&nbsp<INPUT NAME=\"b_d_principal\" id=\"b_d_principal\" TYPE=\"TEXT\" VALUE=\"\" size=\"10\" $HIGHLIGHT readonly>";
...
echo "<td colspan=\"1\">Balance OverDue Principal:<td>Rs.&nbsp<INPUT NAME=\"b_od_prin\" id=\"b_od_prin\" TYPE=\"TEXT\" VALUE=\"\" size=\"10\" $HIGHLIGHT readonly></tr>";
