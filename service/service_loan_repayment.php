...
$sql_statement="SELECT getLoanInterest('$account_no',CURRENT_DATE,'ser') as int";
}
else{
if($comp_method=='in')
$sql_statement="SELECT getLoanDtl('$action_date','$account_no','$loan_serial_no')";
else
$sql_statement="SELECT getLoanInterest('$account_no','$action_date','ser') as int";
}
}
echo $sql_statement;
...
echo "<html>";
echo "<head>";
echo "<title>Service Loan Details </title>";
...
if(pg_NumRows($result)>0){
	$color="GREEN";
	echo "<tr><th bgcolor=YELLOW colspan=\"10\">Service Loan Details of [$account_no] as on ".$action_date."</th>";
	echo "<tr>";
	...
	if(!empty($loan_sl_no)){
	echo "<td align=Center bgcolor=$color><a href=\"service_loan_repayment.php?menu=$menu&loan_no=$account_no&ln_sl=$loan_sl_no&op=r&action_date=$action_date\">Repay</td>";
	}
}
}
else{
 echo "<h1><center>Your dont have any Service Loan!!!!!!!!!!!</h1></center>";
    }
  }
else{
 echo "<h1><font color=RED><center>Your dont have any Service Loan for Payment!!!!!!!!!!!</h1></center>";
    }
}
//==================================FOR REPAY FORM=========================================
if($op=='r'){
...
echo "<tr><th bgcolor=\"#808000\" colspan=\"4\"><font size=+2>Service Loan Repayment Form of [$account_no]</font>";
echo "<FORM NAME=\"orderform\" method=\"POST\" action=\"service_loan_repayment.php?menu=$menu&op=i&ln_sl=$ln_sl&loan_no=$account_no&id=$id\" onSubmit=\"return varify();\">";
...
echo "<td>Total Upto $action_date:<td>Rs.&nbsp;<INPUT NAME=\"total_d\" id=\"total_d\" TYPE=\"TEXT\" VALUE=\"".($due_principal+$odue_i+$due_i+$overdue_principal)."\" $HIGHLIGHT size=\"10\" readonly onchange=\"return check(event)\" >";
...
//echo $sql_statement;
	//For GL ENTRY-------------------------------------
	$sql_statement.=";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','ser','$action_date','$fy','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$re_tot_amt,'Dr','Service Loan repay')";
