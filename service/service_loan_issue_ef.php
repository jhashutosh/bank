...
$app_amount=$_REQUEST['applied_amount'];
...
echo "<FORM NAME=\"form1\" method=\"POST\" action=\"service_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify();\">";
echo "<table bgcolor=\"#60EEA8\" width=\"80%\" align=\"CENTER\" border=1>";
echo "<tr><th bgcolor=\"#EB9B70\" colspan=\"6\"><font size=+2>Service Loan Issue Form[$account_no]</font>";
echo "<tr><td>Issuing Date:<td><INPUT NAME=\"action_date\" TYPE=\"TEXT\" VALUE=\"".date('d/m/Y')."\" $HIGHLIGHT size=\"15\" id=\"action_date\" readonly>";

echo "<td>Add Security Amount:<td>";
if(empty($loan_sl_no)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=ser','win2')\">";

echo "<tr><td align=\"\" colspan=\"3\"><td align=\"CENTER\"><input type=\"SUBMIT\" VALUE=\"    Go   \" DISABLED>";
}
else{
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=ser','win2')\" DISABLED>";

echo "<tr><td>Due Interest Rate:<td><INPUT NAME=\"due_int\" TYPE=\"TEXT\" VALUE=\"13\" $HIGHLIGHT size=3> &nbsp;%";
echo "<td>Over Due Interest Rate:<td><INPUT NAME=\"over_due_int\" TYPE=\"TEXT\" VALUE=\"14\" $HIGHLIGHT size=\"3\" > &nbsp;%";
if($sum_amount>='30000')
{echo "<tr><td>Applied Amount:<td>Rs.&nbsp<INPUT NAME=\"applied_amount\" id=\"applied_amount\" TYPE=\"TEXT\" VALUE=\"500000\" size=\"10\" $HIGHLIGHT readonly>";}
if($sum_amount<'30000')
{echo "<tr><td>Applied Amount:<td>Rs.&nbsp<INPUT NAME=\"applied_amount\" id=\"applied_amount\" TYPE=\"TEXT\" VALUE=\"300000\" size=\"10\" $HIGHLIGHT readonly>";}
...
echo "<tr><td>Repayment Period:<td><INPUT NAME=\"period\" id=\"period\" TYPE=\"TEXT\" VALUE=\"\" size=\"4\" $HIGHLIGHT onkeypress=\"return numbersonly(event);\"> &nbsp;Months";

//****************************************rajesh*************************************************

echo "<td align=\"left\">Payment Mood : </td>";
...
echo "<tr><td colspan=\"4\" align=\"right\"><input type=\"SUBMIT\" VALUE=\"    Go   \" $HIGHLIGHT>";
}
echo "</FORM>";
}
...
if($_REQUEST['op']=='v'){
	...
	echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"service_loan_issue_ef.php?menu=$menu&op=i\">";
echo "<table bgcolor=\"#87CEFA\" width=\"80%\" align=\"CENTER\" border=1>";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>Service Loan Issue Form For verify[$account_no]</font>";
echo "<tr><td>Issuing Date:<td><INPUT NAME=\"action_date\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['action_date']."\" $HIGHLIGHT size=\"15\" readonly>";
echo "<td>Repayment Period:<td><INPUT NAME=\"period\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['period']."\" size=\"4\" $HIGHLIGHT readonly> &nbsp; Months";
...
echo "<tr><td>Applied Amount:<td>Rs.&nbsp<INPUT NAME=\"applied_amount\" TYPE=\"TEXT\" VALUE=\"$app_amount\" size=\"10\" $HIGHLIGHT readonly>";
...
$sql="select repayment_mode,installment_mode from loan_repayment where loan_type like 'ser'";
echo $sql_statement;
$res=dBConnect($sql);
...
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,period,payment_term,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status,loan_compt_status) VALUES ('$loan_sl_no','ser','$id','$account_no', '$fy1','$action_date','$repay_date',$period,'$payment_term',$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d','$repayment_mode')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','ser','$action_date','$fy1','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$ch_no','$cheque_dt')";
//gl_ledger_dtl ===== Dr.
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$issue_amount,'Dr','Service Loan issue')";
//gl_ledger_dtl ===== Cr.

			if($t_status=='c')
			{
				$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$issue_amount,'Cr','Service Loan issue')";
			}
			if($t_status=='y')
			{
		...
		echo "T Sattus is - ". $_REQUEST['t_status'];
		echo $sql_statement;
		...
		echo "<div class='failed'><h3><font color=\"white\">Failed to update database.</font></h3></div>";
			...
			echo "<font size=+1><a href=\"../main/set_account.php?menu=ser&account_no=$account_no\">Click</a> here to go Statement</div>"; 
		}
	}
    }
}
