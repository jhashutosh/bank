...
if (IsPNumeric(amount)){
    if(op=='w'){
		...
		if(document.getElementById("particulars").value=='cheque'){
			//amount=parseFloat(amount);
			bal=parseFloat(bal);
			if(bal<500){
			alert('Required Minimum Balance is Lower than 500 for third party Cheque')
			return false;
			}
		}
...
if($withdrawal==1){
	...
	echo "<td align=\"left\">Withdrawals:<td><input type=\"TEXT\" name=\"withdrawals\" size=\"20\" id=\"amount\" value=\"\" $HIGHLIGHT><br>";
...
if($deposit==1){
	...
	echo "<td align=\"left\">Deposits:<td><input type=\"TEXT\" name=\"deposits\" size=\"20\" id=\"amount\" value=\"\" $HIGHLIGHT><br>";
