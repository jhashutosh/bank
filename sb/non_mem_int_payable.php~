...
echo "<hr>";
echo "<form method=\"POST\" action=\"non_mem_int_payable.php?menu=$menu\" method=\"POST\" name=f1>";
...
$sql_statement="SELECT account_no, sum(amount) AS total_int FROM sb_detail_view WHERE particulars='int.' and gl_mas_code='14301' AND action_date BETWEEN '$starting_date' AND '$ending_date' GROUP BY account_no ORDER BY cast(substring(account_no,3) as int)";
