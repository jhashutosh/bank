...
echo "<hr>";
echo "<form method=\"POST\" action=\"sb_interest_summary.php?menu=$menu\" method=\"POST\" name=f1>";
...
$sql_statement="SELECT account_no, sum(amount) AS total_int FROM sb_detail_view WHERE particulars='int.' AND action_date BETWEEN '$starting_date' AND '$ending_date' GROUP BY account_no ORDER BY cast(substring(account_no,3) as int)";
...
echo "<tr>";
$color="cyan";
echo "<td align=center bgcolor=$color><B>Total:$j</B></td>";
