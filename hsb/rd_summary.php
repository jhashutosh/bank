...
echo "<font size=+2>Summary Statement - Recurring Deposit";
echo "</font>";
echo "<hr>";
echo "<form method=\"POST\" action=\"rd_summary.php?menu=$menu\">";
...
echo "<hr>";
$sql_statement="select action_date,SUM(case when dr_cr='Cr' THEN amount ELSE 0 END) as deposit,SUM(case when dr_cr='Dr' THEN amount ELSE 0 END) as withdrawal from rd_detail_view group by action_date having action_date BETWEEN '$starting_date' AND '$ending_date'  ORDER BY action_date DESC";
...
echo "<td align=center bgcolor=$color><a href=\"../general/scroll.php?menu=rd&current_date=".$row['action_date']."\"onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=100,left=150, width=1210,height=1000'); return false;\">Details</a></td>";
