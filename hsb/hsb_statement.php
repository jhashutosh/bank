...
echo "<h3>Account Statement of DDS Deposit [$account_no]";
echo "</h3><hr>";
//echo "menu is:$menu";
...
if($flag==1){
echo "<hr>";
$sql_statement="SELECT * FROM customer_hsb WHERE account_no='$account_no'";
...
//echo "<td align=right bgcolor=$color>".round($row['maturity_amount']-($row['principal']*$row['period']))."</td>";
//echo "<td align=right bgcolor=$color>".($row['maturity_amount']-($row['principal'])/2)."</td>";
...
$sql_statement="SELECT * FROM hsb_detail_view where account_no='$account_no' and action_date<=current_date ORDER BY action_date DESC,entry_time DESC";
//echo $sql_statement;
$result=dBConnect($sql_statement);
if(pg_NumRows($result)==0) {
