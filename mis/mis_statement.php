...
//$sql_statement="SELECT * FROM customer_mis WHERE account_no='$account_no' and (status='op' or status='cl')";
$sql_statement="SELECT * FROM customer_mis a WHERE account_no='$account_no' and entry_time=(select max(entry_time) from customer_mis b where a.account_no=b.account_no) and (status='op' or status='cl')";
//echo $sql_statement;
...
//---------------------------------------------------------------------------------------------
if(!empty($row['transaction_month'])){
echo "<p style='background-color:white;height:30px' align=center >Before Opening Rs. <font color='green'>".$row['transaction_amount']."</font> Interest Received for <font color='green'>".$row['transaction_month']."</font> months</p>";
}
...
echo "<table valign=\"top\" width=\"100%\">";
$sql="SELECT coalesce(SUM(debit),0) AS balance FROM mas_gl_tran where action_date<=current_date and account_no='$account_no'
and gl_mas_code >'6'";
 //echo $sql;
	$res=dBConnect($sql);
		        $balance=pg_result($res,'balance');
