...
if (empty($op)){
echo "<hr>";
echo "<form method=\"POST\" name=\"f1\" action=\"mini_opening_bal.php?op=i&cid=$cid\" onSubmit=\"return check();\">";
...
echo "<td align=\"left\">Customer Name:</td><td>". $_REQUEST['name']."[$cid]</td>";
echo"<td align=\"left\">Mini Master:<font color=\"RED\">*</font><td>";
$mini_dd_sql="select distinct mm.id,mini_name  from lc_mini_master mm,lc_mini_customer_link mcl where mcl.id_mini_master=mm.id and mcl.id_customer_master='$cid'";
$dd_res=dBConnect($mini_dd_sql);
echo"<select name=mini_id><option value='0'>Select</option>";
for($j=0;$j<pg_NumRows($dd_res);$j++){
$row=pg_fetch_array($dd_res,$j);
