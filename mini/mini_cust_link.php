...
echo "<center><font color=BLUE size=+3><b>MINI Customer Link</b></font></center>";
$color="LIGHTGREEN";
if (empty($op)){
echo "<hr>";
echo "<form method=\"POST\" name=\"f1\" action=\"mini_cust_link.php?op=i\" onSubmit=\"return check();\">";
echo "<table bgcolor=BLACK align=center width=80% >";
echo "<tr><TH colspan=4 bgcolor=BLUE><font color=WHITE size=+2 align=\"center\"><b>MINI Customer Link form";
...
echo "<td align=\"left\">Customer Name: <font color=\"RED\">*</font><td><input type=\"TEXT\" name=\"customer_id\" size=\"30\" id=\"customer_id\"    onfocus=\"show(this.value)\" onchange=\"show(this.value)\" $HIGHLIGHT>";
...
echo "</form>";
echo "<hr>";
//we had made a software which is serves at pacs with  guildline of NABARD 
//--------------------------------------------------------------------------------------------------------------------------------------
$sql_statement="SELECT * FROM LC_Mini_Customer_Link";
//echo $sql_statement;
$result=dBConnect($sql_statement);
if (pg_NumRows($result)>0){
echo "<table bgcolor=BLACK align=center width=80% >";
echo "<tr><TH colspan=5 bgcolor=BLUE><font color=WHITE size=+2 align=\"center\"><b>MINI Customer Link Details";
echo "<tr bgcolor=GREEN><th> Mini name<th>Customer Name<th> Link Date<th>End Date<th>Operator Code</tr>";
$color=$TCOLOR;
for($j=0; $j<pg_NumRows($result); $j++){
	...
	echo "<td>".getName('customer_id',$row['id_customer_master'],'name1','customer_master')."</td>";
...
$sql_statement="SELECT LC_Mini_Customer_Link_Save_Fnc($mini_id,'$customer_id','$link_dt','$end_dt','$staff_id',CAST('$end_dt'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP)) as fnc";
//echo $sql_statement;
$result=dBConnect($sql_statement);
if (pg_NumRows($result)>0){
	...
	if($rns==0){
		...
		header('location:mini_cust_link.php');
		}
	}
}
echo "</body>";
