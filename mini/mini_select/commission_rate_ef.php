...
echo "<tr><th colspan=\"5\" bgcolor=green><b><font color=White><blink>Commission</blink> rate entry form</font></b></th>";
echo "<form name=\"f1\" method=\"POST\" action=\"commission_rate_ef.php?menu=$menu&op=i\">";
echo "<tr><td align=\"left\"><font color=red size=\"2\">*</font>Commission rate:<input type=\"TEXT\" name=\"comm_rt\" size=5 value=\"\" $HIGHLIGHT>";
...
$sql_statement="select LC_Operator_Commission_Master_Vld_Fnc('$op_dt') as vld";
		...
		if($return!=0)
			{
			$sql_statement="SELECT LC_Operator_Commission_Master_Save_Fnc('$comm_rt','$op_dt','$staff_id','$time') as integer";		
			$result=dBConnect($sql_statement);
			$row=pg_NumRows($result);
			if(pg_NumRows($result)>0)
				{
...
if(pg_NumRows($result)==0) {
echo "<center>";
echo "<h4><font size=5 color=green><blink>Please enter Commission rate !!!</blink></font></h4>";
echo "</center>";
} 
else 
{
