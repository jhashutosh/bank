...
$pemp_id=$_REQUEST['id']  ;
$invst=$_REQUEST['invst'];
$arr=explode('|',$invst);
$pid_emp_investment_mas=$arr[0] ;
$pdeposit_no=$_REQUEST['de_no'] ;
$cer_no=(empty($_REQUEST['cer_no']))?null:$_REQUEST['cer_no'] ;
$pbank_name=$_REQUEST['bnk_name']  ;
$pbank_dtl=$_REQUEST['bnk_dtls'] ;
$pinv_mode=(empty($_REQUEST['prds_in']))?null:$_REQUEST['prds_in'] ;
$pinv_period=(empty($_REQUEST['period']))?0:$_REQUEST['period'] ;
$pdt_of_issue=$_REQUEST['inv_dt'] ;
$parts=explode('/',$pdt_of_issue);
$pdt_of_issue=$parts[1].'/'.$parts[0].'/'.$parts[2];
//echo $_REQUEST['mtr_dt'];
$pdt_of_maturity=( empty($_REQUEST['mtr_dt']) or $_REQUEST['mtr_dt']=='Invalid Date')?$pdt_of_issue:$_REQUEST['mtr_dt'] ;
//echo $pdt_of_issue;
//echo $pdt_of_maturity;
$pamount=$_REQUEST['dp_amnt']  ;
$pint_mode=(empty($_REQUEST['inst_mode']))?null:$_REQUEST['inst_mode'] ;
$pint_percent=$_REQUEST['int_rt']  ;
$pint_amt=(empty($_REQUEST['int_amnt']))?0:$_REQUEST['int_amnt'] ;
$poperator_code=verifyAutho();
$pentry_time=date('d/m/Y H:i:s ');
if($pdt_of_maturity!=$pdt_of_issue){
$date_sql="select  cast(substr('$pdt_of_maturity',5,11) as date)";
//echo $date_sql;
$r=dBConnect($date_sql);
$row=pg_fetch_array($r,0);
$mtrty_dt=$row['substr'];}
else $mtrty_dt=$pdt_of_maturity;

$mat_amt=(empty($_REQUEST['t_m_amnt']))?0:$_REQUEST['t_m_amnt'];
//echo $mtrty_dt;
$sql="select emp_pforgratuity_sb_hdr_save_fnc('$acno','$cer_no','$pac_type',$emp_id,$pid_emp_investment_mas,'$pdeposit_no','$pbank_name','$pbank_dtl','$pinv_mode',$pinv_period,'$pdt_of_issue','$mtrty_dt',$pamount,'$pint_mode',$pint_percent,$pint_amt,'$poperator_code','$pentry_time',$mat_amt)";
$res=dBConnect($sql);
//echo $sql;
}
if($type=='po'){
...
$sql1="select w.pf_ac_no,w.emp_id,w.op_bal,w.pf_dep as dep,coalesce(z.bn_gain,0)+coalesce(y.lic_gain,0)+coalesce(x.po_gain,0)+w.pf_dep as bal  from 
...
$c_b="select coalesce(pf_dep,0)+coalesce(bk_with,0)+coalesce(int,0)+coalesce(po_with,0)+coalesce(lic_with,0)-coalesce(bk_dep,0)-coalesce(po_dep,0)-coalesce(lic_dep,0) as current_balance from (
(select op_bal+total_empl_cont_pf_amt+total_emplee_cont_pf_amt as pf_dep,emp_id from emp_pf_dtl) as a
left outer join
(select sum(coalesce(withdrawl_amount,0)) as bk_with,sum(coalesce(deposit_amount,0)-coalesce(int_amt,0)) as bk_dep ,sum(coalesce(int_amt,0)) as int,emp_id from emp_pforgratuity_sb_hdr group by emp_id) as c 
on a.emp_id=c.emp_id
left outer join
(select sum(coalesce(withdrawl_amount,0)) as po_with,sum(coalesce(tot_amount,0)) as po_dep ,emp_id from emp_pforgratuity_po_hdr group by emp_id ) as b
on a.emp_id=b.emp_id
left outer join
(select sum(coalesce(withdrawl_amount,0)) as lic_with,sum(coalesce(deposit_amt,0)) as lic_dep ,emp_id from lic_invst group by emp_id ) as d
on a.emp_id=d.emp_id
...
var desc=s.split('|');
alert (desc[1])
if(desc[1]=='sb' || desc[1]=='SB')	
{
	document.getElementById('period_hide').style.display='none';
	document.getElementById('mat_date_hide').style.display='none';
	document.getElementById('cer_no_dis').style.display='none';
...
{
	document.getElementById('period_hide').style.display='';
	document.getElementById('mat_date_hide').style.display='';
	document.getElementById('cer_no_dis').style.display='';
...
echo"<table valign=\"top\"width='100%' align='center'>";
echo"<tr><th colspan='4'bgcolor='#839EB8'><font color='#043C70' size='2'>Bank Investment</font></th></tr>";
echo "<td  colspan=\"2\"><font color='black' size='2' align='left'>Investment Type&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
echo "<select name=\"invst\" id=\"invst\" onchange=\"show_hide(this.value);\"> <option value=''> Select</option>";
$sql="select distinct(invst_desc),upper(invst_desc) as invst_desc,id from emp_investment_mas where trim(type) like 'bank'";
//echo $sql;
$res=dBConnect($sql);
for($j=0;$j<pg_NumRows($res);$j++){
$row=pg_fetch_array($res,$j);

echo"<option value=".$row['id']."|".$row['invst_desc'].">".$row['invst_desc']."</option>";
}
echo"</select></td>";
echo "<td ><font color='black' align='left'>Deposit No.&nbsp;:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";
...
echo "<td id='cer_no_dis'><font color='black'size='2' align='left'>Certificate No. &nbsp;:&nbsp;";
echo"<input type=\"TEXT\" name=\"cer_no\" size=\"10\" value=\"\" id=\"cer_no\" $HIGHLIGHT1 ></td>";
echo"</tr>";
echo"<tr>";
echo "<td  colspan=\"2\"><font color='black'size='2' align='left'>Deposit Amount&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
echo"<input type='text' name='dp_amnt' id='dp_amnt' size='5' $HIGHLIGHT1 onkeypress=\"return numbersonly(event);\"></td>";
echo"<td><font color='black'size='2' align='left'>Bank Name&nbsp;:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";
echo"<input type='text' name='bnk_name' size='15' $HIGHLIGHT1 onkeypress=\"return charonly(event);\"></td>";
echo "<td><font color='black'size='2' align='left'>Bank Details&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
echo"<input type='text' name='bnk_dtls' size='15' $HIGHLIGHT1></td></tr>";

echo "<tr><td colspan=\"2\"><font color='black'size='2' align='left'>Deposit Date&nbsp;:&nbsp;";
...
echo"</td>";
echo "<td  colspan=\"1\" id='period_hide'><font color='black'size='2' align='left'>Periods in&nbsp;:";
...
echo"<td id='mat_date_hide'>Maturity Date : <input type=\"TEXT\" name=\"mtr_dt\" size=\"15\" value=\"\" id=\"mtr_dt\" onClick=\"cal_days(this.form)\" onKeyup=\"cal_days(this.form)\" $HIGHLIGHT1 >";
...
echo"<tr><th colspan='4'bgcolor='#839EB8'><font color='#043C70' size='2' >Interest Details</font></th></tr>";
echo "<tr>";
...
echo "<td id='int_dis1' colspan=\"1\"align='left'><font color='black'size='2' align='left'>Interest Amount&nbsp;:&nbsp;";
echo"<input type='text' name='int_amnt'  id='int_amnt' size='10' $HIGHLIGHT1 onkeypress=\"return numbersonly(event);\"></td>";
echo "<td align='left'  id='int_dis2' ><font color='black' size='2' >Total Maturity Amount&nbsp;:&nbsp;";
...
echo "<th bgcolor='#BCA9F5' width=\"15%\"><font color='#043C70' size='2'>Deposit Amount</font></th>";
...
echo "<th bgcolor='#BCA9F5' width=\"20%\"><font color='#043C70' size='2'>Transaction</font></th>";
echo "<th bgcolor='#BCA9F5' width=\"10%\"><font color='#043C70' size='2'>Operation</font></th>";
...
echo "<td  bgcolor='$color' align='center' colspan=\"1\"  width=\"15%\"><font color='$fcolor' size='2'>".$row['deposit_amount']."</font></td>";
...
if($row['invst_desc']!='sb')
echo "<td  bgcolor='$color' align='center'  colspan=\"1\"  width=\"15%\"><font color='$fcolor' size='2'>".$row['dt_of_maturity']."</font></td>" ;
else
echo "<td  bgcolor='$color' align='center'  colspan=\"1\"  width=\"15%\"></td>" ;
if(($row['invst_desc']=='rd') and $mt==0)
echo"<td width='20%' align='right'>
<a href=\"../payroll/bank_dep.php?type=".$row['invst_desc']."&gl_mas_code=".$row['gl_mas_code']."&c_id=$c_id&emp_id=$emp_id&deposit_no=$deposit_no\" target=_parent onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=350, width=500,height=150'); return false;\"><font color='#0085FF' style='bold',style='italics'>Deposit</a></td>";
elseif ($row['invst_desc']=='sb'and $mt==0) {
	echo"<td width='20%' align='right'>
<a href=\"../payroll/bank_dep.php?opr=d&liab_code=".$pfr['gl_mas_code']."&type=".$row['invst_desc']."&gl_mas_code=".$row['gl_mas_code']."&c_id=$c_id&emp_id=$emp_id&deposit_no=$deposit_no\" target=_parent onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=350, width=500,height=150'); return false;\"><font color='#0085FF' style='bold',style='italics'>Deposit</a>
||
<a href=\"../payroll/bank_dep.php?opr=w&liab_code=".$pfr['gl_mas_code']."&type=".$row['invst_desc']."&gl_mas_code=".$row['gl_mas_code']."&c_id=$c_id&emp_id=$emp_id&deposit_no=$deposit_no\" target=_parent onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=350, width=500,height=150'); return false;\"><font color='#0085FF' style='bold',style='italics'>Withdrawl</a></td>";

}
else echo "<td width='20%' align='right'>Can't Deposit</td>";
//echo "<td><a href='#' class='popupClass'>Edit</a>"; edit
echo "<td width='10%' align='right'>";
if($mt==0) 
echo"<a href=\"../payroll/mature_invst.php?type=bn&c_id=$c_id&gl_mas_code=".$row['gl_mas_code']."&emp_id=$emp_id&deposit_no=$deposit_no\" target=_parent onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=350, width=500,height=150'); return false;\"><font color='#0085FF' style='bold',style='italics'>$mat Mature </a>";
else echo "<font color='green'> Withdrawl Amount </font>".$row['withdrawl_amount']; 
...
$sql="select distinct(invst_desc) ,upper(invst_desc) as invst_desc,id from emp_investment_mas where trim(type) like 'po'";
$res=dBConnect($sql);
for($j=0;$j<pg_NumRows($res);$j++){
$row=pg_fetch_array($res,$j);
//echo $sql;
...
echo"<table valign=\"top\"width='100%;' align='center' class='report'>";
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$sql=" select p.*,m.invst_desc,m.gl_mas_code,case when  dt_of_maturity-current_date <= 0 then 0 
 	 when  dt_of_maturity-current_date < 3 then 1 
 	 else 2 end as cl ,
 	 case when withdrawl_amount is null then 0 else 1 end as mt
 from emp_pforgratuity_po_hdr p, emp_investment_mas m where p.id_emp_investment_mas=m.id and p.emp_id=$emp_id";
$res=dBConnect($sql);
//echo $sql;
$color==$TCOLOR;
for($j=0;$j<pg_NumRows($res);$j++){
...
if($mt==0) 
echo"    ||   <a href=\"../payroll/mature_invst.php?type=po&c_id=$c_id&emp_id=$emp_id&saving_regno=$saving_regno&gl_mas_code=".$row['gl_mas_code']."\" target=_parent onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=350, width=500,height=150'); return false;\"><font color='#0085FF' style='bold',style='italics'>Mature </a>";
echo"</td>";
echo"</tr>";
}
echo"</table>";
...
$sql="select distinct(invst_desc) ,upper(invst_desc) as invst_desc,id from emp_investment_mas where trim(type) like 'lic'";
$res=dBConnect($sql);
for($j=0;$j<pg_NumRows($res);$j++){
$row=pg_fetch_array($res,$j);
//echo $sql;
...
echo"<table valign=\"top\"width='100%;' align='center' class='report'>";
//
$sql="select l.*,m.invst_desc,m.gl_mas_code,case when  dt_of_maturity-current_date <= 0 then 0 
  when  dt_of_maturity-current_date < 3 then 1 
  else 2 end as cl ,
  case when withdrawl_amount is null then 0 else 1 end as mt
 from emp_pforgratuity_lic_hdr l ,emp_investment_mas m where l.id_emp_investment_mas=m.id and l.emp_id=$emp_id;";
$res=dBConnect($sql);
//echo $sql;
$color==$TCOLOR;
for($j=0;$j<pg_NumRows($res);$j++){
...
if($mt==0) 
echo"||   <a href=\"../payroll/mature_invst.php?type=lic&c_id=$c_id&emp_id=$emp_id&pol_no=$pol_no&gl_mas_code=".$row['gl_mas_code']."\" target=_parent onClick=\"window.open(this.href,'_blank','toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=350, width=500,height=250'); return false;\"><font color='#0085FF' style='bold',style='italics'>$mat Mature </a>";
echo"</td>";
echo"</tr>";
}
