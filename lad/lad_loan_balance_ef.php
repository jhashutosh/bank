...
$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','lad','$curr_date','$fy','$remarks', '$staff_id', CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
		
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$balance_p,'Dr','opening LAD')";
...
echo "<tr><th bgcolor=\"#006400\" colspan=\"4\"><font size=+2>LAD Issuing Form of [$account_no]</font>";
echo "<FORM NAME=\"f1\" method=\"POST\" action=\"lad_loan_balance_ef.php?menu=$menu&op=i\">";
echo "<TR><td>Issuing Date:<td><INPUT NAME=\"action_date\" TYPE=\"TEXT\" VALUE=\"$action_date\" $HIGHLIGHT size=\"12\" id=\"action_date\" $ON>";
...
echo "<tr><td>Due Interest Rate:<td>Rs.&nbsp;<INPUT NAME=\"d_int_r\" TYPE=\"TEXT\" VALUE=\"12\" $HIGHLIGHT maxlength=\"6\" size=3>&nbsp;%";
echo "<td>Over Due Interest Rate:<td>Rs.&nbsp;<INPUT NAME=\"od_int_r\" TYPE=\"TEXT\" VALUE=\"14\" $HIGHLIGHT size=\"3\" maxlength=\"6\">&nbsp;%";
