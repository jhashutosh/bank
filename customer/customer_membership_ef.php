...
$mem_id_sql="select case when (MAX(cast(substr(membership_no,3,length(membership_no)) as int))+1) is null then 1 else (MAX(cast(substr(membership_no,3,length(membership_no)) as int))+1) end id from membership_info where membership_no like 'M-%'";
$cust_id_res=dBConnect($mem_id_sql);
$m_id=pg_fetch_array($cust_id_res,0);
$member_id='M-'.$m_id['id'];


echo "<td>Membership No. : <td><input type=\"hidden\" name=\"ac_ty\" id=\"ac_ty\" size=5 value=\"".strtoupper($type)."-\" readonly><input type=\"TEXT\" name=\"m_no\" id=\"m_no\" size=15 value=\"$member_id\"onkeypress=\"return numbersonly(event);\"  onkeyup=\"return showaccounthints(event);\" onblur=\"return showaccounthints(event);\" onselect=\"return alert1(this);\" onclick=\"return click1(this);\" $HIGHLIGHT ><span id=\"hintspan\"></span>";
...
echo "<td>Per Share :<td> Rs. &nbsp; <input type=text size=1 name=val_of_share id=val_of_share value=1   $HIGHLIGHT>";
