...
$sql_statement="truncate table debtors_list;SELECT add_debtors_summary('$start_date','$end_date') as debtors";
//echo $sql_statement;
...
//$flag=1;
if($flag==1){
//$sql_statement="SELECT * FROM debtors_list1 order by CAST(SUBSTR(account_no,3,LENGTH(account_no)) AS numeric) DESC";
//$sql_statement="SELECT * FROM debtors_list ORDER BY cast(substring(account_no,3) as int) desc";
...
echo "<tr><th bgcolor=\"GREEN\" colspan=\"9\" align=center><font color=\"white\">SB Debtors Summary As On $end_date</font>";
...
echo "<td align=right bgcolor=$color><font color=$fColor><b>".$row['status']."</b></font></td>";

}

echo "<tr>";
$color="cyan";
echo "<td align=center bgcolor=$color colspan=\"3\"><B>Total : $j Accounts Found(Total Open a/c:$no_op and Total Close a/c:$no_cl)</B></td>";
