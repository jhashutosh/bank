...
echo "<FORM NAME=\"form1\" method=\"POST\" action=\"mt_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify();\">";
echo "<table bgcolor=\"#4385CC\" width=\"80%\" align=\"CENTER\" >";
echo "<tr><th bgcolor=\"#CE3CC1\" colspan=\"6\"><font size=+2>MT Issue Form[$account_no]</font>";
echo "<tr ><td>Issuing Date:<td><INPUT NAME=\"action_date\" TYPE=\"TEXT\" VALUE=\"".date('d/m/Y')."\" $HIGHLIGHT size=\"15\" id=\"action_date\">";
echo "<td>Period:<td>Rs.&nbsp<INPUT NAME=\"period\" id=\"period\" TYPE=\"TEXT\" VALUE=\"\" size=\"4\" $HIGHLIGHT> &nbsp;Month";
/*
...
*/
echo "<tr><td>Due Interest Rate:<td><INPUT NAME=\"due_int\" TYPE=\"TEXT\" VALUE=\"7\" $HIGHLIGHT size=3> &nbsp;%";
echo "<td>Over Due Interest Rate:<td><INPUT NAME=\"over_due_int\" TYPE=\"TEXT\" VALUE=\"11\" $HIGHLIGHT size=\"3\" > &nbsp;%";
//echo "<tr><td>Security Amount:<td>Rs.&nbsp<INPUT NAME=\"security_amount\" TYPE=\"TEXT\" VALUE=\"$sum_amount\" size=\"14.5\" $HIGHLIGHT READONLY> <a href=document_view.php?menu=pl&account_no=$account_no>View</a>";
...
echo "<table bgcolor=\"#B8A1D3\" width=\"80%\" align=\"CENTER\" >";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>MT Issue Form For verify[$account_no]</font>";
echo "<tr><td>Issuing Date:<td><INPUT NAME=\"action_date\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['action_date']."\" $HIGHLIGHT size=\"15\" readonly>";
echo "<td>Repayment Period:<td>Rs.&nbsp<INPUT NAME=\"period\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['period']."\" size=\"10\" $HIGHLIGHT readonly> &nbsp; Months";
//echo "<td>Add Security Amount:<td>";
//echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=mt','win2')\" DISABLED>";
...
//echo "<tr><td>Security Amount:<td>Rs.&nbsp<INPUT NAME=\"security_amount\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['security_amount']."\" size=\"10\" $HIGHLIGHT READONLY> <a href=document_view.php?menu=pl&account_no=$account_no>View</a>";
//echo "<td>Repayment Period:<td>Rs.&nbsp<INPUT NAME=\"period\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['period']."\" size=\"10\" $HIGHLIGHT readonly> &nbsp; Months";
...
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,period,payment_term,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status,loan_compt_status) VALUES ('$loan_sl_no','mt','$id','$account_no', '$fy','$action_date','$repay_date',$period,'$payment_term',$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d','$repayment_mode')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','mt','$action_date','$fy','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$ch_no','$cheque_dt')";
//gl_ledger_dtl ===== Dr.
...
//gl_ledger_dtl ===== Cr.
		...
		echo "<div class='failed'><h3><font color=\"white\">Failed to update database.</font></h3></div>";	
			...
			echo "<font size=+1><a href=\"../main/set_account.php?menu=mt&account_no=$account_no\">Click</a> here to go Statement"; 
		}
