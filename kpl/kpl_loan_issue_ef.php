...
function varify()
{
	...
	var rep_repay_date=document.getElementById("repay_date").value;
	
	if(rep_repay_date=='')
	{
		alert("Repayment Date Should Not Be Blank !!!!");
		document.getElementById("repay_date").value='';
		document.getElementById("repay_date").focus();
		return false;
	}
...
echo "<FORM NAME=\"form1\" method=\"POST\" action=\"kpl_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify(this.form);\">";
echo "<table bgcolor=\"#FDF5E6\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>KVP Loan Issuing Form[$account_no]</font>";
echo "<tr><td>Issuing Date:<td><INPUT NAME=\"action_date\" ID=\"action_date\" TYPE=\"TEXT\" VALUE=\"$action_date\" $HIGHLIGHT size=\"13\" >";
echo "<td>Add Security Amount:<td>";
if(empty($loan_sl_no)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=kpl','win2')\">";
...
echo "<tr><td>Security Amount:<td>Rs.&nbsp<INPUT NAME=\"security_amount\" id=\"security_amount\" TYPE=\"TEXT\" VALUE=\"$sum_amount\" size=\"10\" $HIGHLIGHT >";
echo "<td>Repayment Date:<td><INPUT NAME=\"repay_date\" TYPE=\"TEXT\" id=\"repay_date\" VALUE=\"\" size=\"13\" $HIGHLIGHT>";
...
$repay_date=$_REQUEST['repay_date'];
	echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"kpl_loan_issue_ef.php?menu=$menu&op=i\">";
echo "<table bgcolor=\"#8FBC8F\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>KVP Loan Issuing Form For verify[$account_no]</font>";
...
echo "<td>Repayment Date:<td><INPUT NAME=\"repay_date\" value=\"$repay_date\"><INPUT NAME=\"repay_date\" TYPE=\"hidden\" VALUE=\"".$_REQUEST['repay_date']."\" size=\"10\" $HIGHLIGHT>";
...
$gl_code=getGlCode4mCustomerAccount($account_no,$action_date);
	...
	if(empty($fy1)){
		echo "<h1><center><b>You Can't Insert any value Into database Because Financial Year already Locked by administrator !!!!!!!!!</h1>";
	 	...
	 	$t_id=getTranId();
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status,payment_term) VALUES ('$loan_sl_no','kpl','$id','$account_no', '$fy1','$action_date','$repay_date',$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d','c')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','kpl','$action_date','$fy1','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$ch_no','$cheque_dt')";
//gl_ledger_dtl ===== Dr.
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$issue_amount,'Dr','kpl issue')";
//gl_ledger_dtl ===== Cr.

		if($t_status=='c')
		{
			$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$issue_amount,'Cr','kpl issue')";
		}
		if($t_status=='y')
		{
