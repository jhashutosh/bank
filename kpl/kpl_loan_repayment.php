...
$sql_statement="SELECT getloaninterest('$account_no','$action_date','kpl') as int";
//echo $sql_statement;
...
function cr_chk()
{
	var d_total=parseInt(document.getElementById('total_d').value);
	var r_amount=parseInt(document.getElementById('amount').value);
		
	var r_amount_prin=document.getElementById('amount').value.length;
		
	if(d_total<r_amount)
	{
		alert('Recovery Principal Amount Exceeds Total Amount')
...
<?php
echo "<title>KVP Loan Details </title>";
...
if(pg_NumRows($result)>0){
	$color="GREEN";
	echo "<tr><th bgcolor=YELLOW colspan=\"9\">KVP Loan Details of [$account_no] as on ".$action_date."</th>";
	echo "<tr>";
	...
	$t_d+=$row['due_int'];
	echo "<td align=right bgcolor=$color>".amount2Rs($row['overdue_int'])."</td>";
	$t_od+=$row['overdue_int'];
	echo "<td align=right bgcolor=$color>".amount2Rs(($row['principal']+$row['due_int']+$row['overdue_int']))."</td>";
	echo "<td align=Center bgcolor=$color><a href=\"kpl_loan_repayment.php?menu=$menu&ln_sl=$loan_sl_no&op=r&action_date=$action_date\">Repay</td>";
     }
echo "<tr><td colspan=3 bgcolor=\"aqua\" align=\"CENTER\"><B>Total: ".($j+1)." Record Found !!!";
echo "<td bgcolor=\"aqua\" align=\"RIGHT\"><B>".amount2Rs($t_p);
echo "<td bgcolor=\"aqua\" align=\"RIGHT\"><B>".amount2Rs($t_d);
echo "<td bgcolor=\"aqua\" align=\"RIGHT\"><B>".amount2Rs($t_od);
echo "<td bgcolor=\"aqua\" align=\"RIGHT\"><B>".amount2Rs($t_p+$t_d+$t_od);
echo "<td bgcolor=\"aqua\" align=\"RIGHT\">";

}
else{
 echo "<h1><center>Your dont have any KVP Loan!!!!!!!!!!!</h1></center>";
    }
  }
else{
 echo "<h1><font color=RED><center>Your dont have any KVP Loan for Payment!!!!!!!!!!!</h1></center>";
    }
}
//==================================FOR REPAY FORM=========================================
if($op=='r'){
$sql_statement="SELECT crop_id,principal,days,round(due_int) as due_int,round(overdue_int) as overdue_int,status FROM loan_cal_int where account_no='$account_no' AND loan_serial_no='$ln_sl'";
//echo $sql_statement;
$result=dBConnect($sql_statement);
if(pg_NumRows($result)>0){
...
echo "<tr><th bgcolor=\"#808000\" colspan=\"4\"><font size=+2>KVP Loan Repayment Form of [$account_no]</font>";
echo "<FORM NAME=\"pForm\" method=\"POST\" action=\"kpl_loan_repayment.php?menu=$menu&op=i&ln_sl=$ln_sl\" onSubmit=\"return cr_chk();\">";
...
echo "<td>Principal :<td>Rs.&nbsp;<INPUT NAME=\"d_principal\" TYPE=\"TEXT\" VALUE=\"$principal\" $HIGHLIGHT size=10 id=\"d_principal\" READONLY >";
...
echo "<td>Total:<td>Rs.&nbsp;<INPUT NAME=\"total_d\" id=\"total_d\" TYPE=\"TEXT\" VALUE=\"".($principal+$odue_i+$due_i)."\" $HIGHLIGHT size=\"10\" >";
...
if(empty($fy)){
	...
	//For GL ENTRY-------------------------------------
	$sql_statement.=";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','kpl','$action_date','$fy','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$amount,'Dr','kpl repay')";
	if($r_i_od>0){
