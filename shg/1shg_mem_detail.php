...
if($record==0) {
	...
	echo "<A HREF=\"shg_member_ef.php?menu=shg&group_no=$account_no\" > New entry </A><BR>";
           }
  } 
else {
...
// Place line comments if you do not need column header.
	...
	echo "<tr>";
	echo "<td align=center bgcolor=$color><A HREF=\"../main/set_account.php?menu=".trim($row['account_type'])."&account_no=".$row['account_no']."\">".$row['account_no']."</A></td>";
	echo "<td align=center bgcolor=$color>".$type_of_account1_array[trim($row['account_type'])]."</td>";
	if(!strcmp(trim($row['account_type']),'sb')){
	$sb_flag=1;
	echo "<td align=right bgcolor=$color>".sb_current_balance($row['account_no'])."</td>";
	}
	if(!strcmp(trim($row['account_type']),'fd')){
	echo "<td align=right bgcolor=$color>".fd_deposit_amount($row['account_no'])."/";
	echo fd_maturity_amount($row['account_no'])."</td>";
	}
	if(!strcmp(trim($row['account_type']),'rd')){
	echo "<td align=right bgcolor=$color>".rd_current_balance($row['account_no'])."/";
	echo rd_maturity_amount($row['account_no'])."</td>";
	}
	if(!strcmp(trim($row['account_type']),'ri')){
	echo "<td align=right bgcolor=$color>".ri_deposit_amount($row['account_no'])."/";
	echo ri_maturity_amount($row['account_no'])."</td>";
	}
	if(!strcmp(trim($row['account_type']),'st')){
	$st_loan=loan_current_balance($row['account_no']);
	echo "<td align=right bgcolor=$color>".$st_loan."</td>";
	}
	echo "<td align=right bgcolor=$color>".$row['remarks']."</td>";
	if(!strcmp(trim($row['account_type']),'mt')){
	$mt_loan=loan_current_balance($row['account_no']);
	echo "<td align=right bgcolor=$color>".$mt_loan."</td>";
	}
...
echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menu&id=$c_id&type=fd\">Fixed deposit</A>";
echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menuid=$c_id&type=rd\">Recurring deposit</A>";
echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menuid=$c_id&type=ri\">Reinvestment</A>";
//echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menuid=$id&type=mis\">MIS</A>";
//echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menuid=$id&type=pl\">Pledge</A>";
echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menuid=$c_id&type=lo\">Loan</A>";
//echo "&nbsp; <A HREF=\"customer_ac_open_ef.php?menu=$menuid=$id&type=kcc\">KCC</A>";
    }
}}
//-----------------------------------------------------------------------------------------
