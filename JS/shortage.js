...
/*----------------transfer-----------------------------*/
$(function(){
	$( "#srtDate" ).datepicker({dateFormat :'dd/mm/yy'});
	$( "#chequeDate" ).datepicker({dateFormat :'dd/mm/yy'});
	//---------------------------
	var url3="./search_cust_type.php?cus=true&type=society";
...
////////////customer search///////////
	$("body").delegate("#s-customer #searchC","change",function(event)
	{
		if($(this).val().length!=0){
			...
			var url4="./search_cust_type.php?des=true&c_id="+$(this).val()+"&type=society";
			if (window.XMLHttpRequest)
		 	{xmlhttp=new XMLHttpRequest();}
			else		
		  	{xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}
...
//////////Submit///////////
	$("body").delegate("#sell_product #finalSubmit","click",function(event)
	{
		if(reqField()==true )
		{
			...
			var srtDate="srtDate="+$("#srtDate").val();
			var c=0;
			...
			if(c==1)
			{
				var sendinfo=branchName+"&"+product_data+"&"+srtDate;
				if (window.XMLHttpRequest) 
				{xmlhttp=new XMLHttpRequest();
				}
				else		
				...
				if (xmlhttp.readyState==4 && xmlhttp.status==200)
				{
					...
					window.location='./stortage.php';
					location.reload('');
				}
				}
				xmlhttp.open("POST",'insert_shortage.php',true);
...
/////////validation function////////////
	function reqField(){
		...
		if($("#srtDate").val()=='' ||$("#srtDate").val()===null)
		{msg+="Enter Date.\n";
		document.getElementById('srtDate').focus();}
