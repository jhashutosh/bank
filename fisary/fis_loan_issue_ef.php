...
$sum_amount=round(($sum_amount*70)/100);
echo "<html>";
echo "<head>";
echo "<title> Fisary Main </title>";
...
echo "<FORM NAME=\"form1\" method=\"POST\" action=\"fis_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify(this.form);\">";
echo "<table bgcolor=\"#ADD8E6\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>Fisary Issuing Form[$account_no]</font>";
...
if(empty($loan_sl_no)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=fis','win2')\">";

echo "<tr><td align=\"\" colspan=\"3\"><td align=\"CENTER\"><input type=\"SUBMIT\" VALUE=\"    Go   \" DISABLED>";

}
else{
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=fis','win2')\" DISABLED>";
echo "<tr><td>Due Interest Rate:<td><INPUT NAME=\"due_int\" TYPE=\"TEXT\" VALUE=\"14\" $HIGHLIGHT size=3> &nbsp;%";
echo "<td>Over Due Interest Rate:<td><INPUT NAME=\"over_due_int\" TYPE=\"TEXT\" VALUE=\"17\" $HIGHLIGHT size=\"3\" > &nbsp;%";
echo "<tr><td>Security Amount:<td>Rs.&nbsp<INPUT NAME=\"security_amount\" id=\"security_amount\" TYPE=\"TEXT\" VALUE=\"$sum_amount\" size=\"10\" $HIGHLIGHT READONLY>";
echo "<td>Period:<td><INPUT NAME=\"period\" id=\"period\" TYPE=\"TEXT\" VALUE=\"\" size=\"3\" $HIGHLIGHT onkeypress=\"return numbersonly(event);\"> &nbsp;Months";
...
if($_REQUEST['op']=='v'){
	...
	echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"fis_loan_issue_ef.php?menu=$menu&op=i\">";
echo "<table bgcolor=\"#8FBC8F\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>Fisary Issuing Form For verify[$account_no]</font>";
...
echo "<td>Repayment Period:<td><INPUT NAME=\"period\" TYPE=\"TEXT\" VALUE=\"".$_REQUEST['period']."\" size=\"3\" $HIGHLIGHT readonly> &nbsp; Months";
...
$gl_code=getGlCode4mCustomerAccount($account_no,$action_date);
	...
	if(empty($fy1)){
		echo "<h1><center><b>You Can't Insert any value Into database Because Financial Year already Locked by administrator !!!!!!!!!</h1>";
	 	...
	 	$t_id=getTranId();
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,period,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status) VALUES ('$loan_sl_no','fis','$id','$account_no', '$fy1','$action_date','$repay_date',$period,$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','fis','$action_date','$fy1','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$ch_no','$cheque_dt')";
//gl_ledger_dtl ===== Dr.
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$issue_amount,'Dr','fis issue')";
//gl_ledger_dtl ===== Cr.

		if($t_status=='c')
		{
			$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$issue_amount,'Cr','fis issue')";
		}
		if($t_status=='y')
		{
