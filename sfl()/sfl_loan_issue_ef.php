...
echo "<FORM NAME=\"form1\" method=\"POST\" action=\"sfl_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify();\">";
echo "<table bgcolor=\"#CDC6F4\" width=\"80%\" align=\"CENTER\" >";
echo "<tr><th bgcolor=\"#BB7382\" colspan=\"6\"><font size=+2>Staff Issue Form[$account_no]</font>";
...
if($_REQUEST['op']=='v'){
	...
	echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"sfl_loan_issue_ef.php?menu=$menu&op=i\">";
echo "<table bgcolor=\"#CDC6F4\" width=\"80%\" align=\"CENTER\" >";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>Staff Issue Form For verify[$account_no]</font>";
...
$sql="select repayment_mode,installment_mode from loan_repayment where loan_type like 'sfl'";
//echo $sql_statement;
$res=dBConnect($sql);
...
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,period,payment_term,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status,loan_compt_status) VALUES ('$loan_sl_no','sfl','$id','$account_no', '$fy','$action_date','$repay_date',$period,'$payment_term',$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d','$repayment_mode')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','sfl','$action_date','$fy','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$ch_no','$cheque_dt')";
//gl_ledger_dtl ===== Dr.
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$issue_amount,'Dr','staff issue')";
//gl_ledger_dtl ===== Cr.
		if($t_status=='c')
		{
			$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$issue_amount,'Cr','staff issue')";
		}
		if($t_status=='y')
		{
