...
function openChild(file,window){
	...
	file=file+"&l_date="+str;
       	childWindow=open(file,window, 'toolbar=no,status=no,location=no,directories=no,resizeable=no,scrollbars=yes, top=300,left=250, width=800,height=200');
...
echo "<FORM  NAME=\"form1\" method=\"POST\" action=\"mt_loan_issue_ef.php?menu=$menu&op=v\" onSubmit=\"return varify(this.form);\">";
echo "<table bgcolor=\"#ADD8E6\" width=\"80%\" align=\"CENTER\" border=1>";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"6\"><font size=+2>MT Issue Form[$account_no]</font>";
echo "<tr bgcolor=\"#ADD8E6\"><td>Issuing Date:<td><INPUT NAME=\"action_date\" TYPE=\"TEXT\" VALUE=\"".date('d/m/Y')."\" $HIGHLIGHT size=\"15\" id=\"action_date\">";

$sql_statement="SELECT status FROM loan_ledger_hrd where account_no='$account_no'";
//echo $sql_statement;
...
if(empty($loan_sl_no) and ($status=='cl' or $status==null)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=mt','win2')\">";

echo "<tr><td align=\"\" colspan=\"3\"><td align=\"CENTER\"><input type=\"SUBMIT\" VALUE=\"    Go   \" DISABLED>";
}
else{
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=mt','win2')\" DISABLED>";
...
echo "<tr><td>Applied Amount:<td>Rs.&nbsp<INPUT NAME=\"applied_amount\" id=\"applied_amount\" TYPE=\"TEXT\" VALUE=\"$sum_amount\" size=\"10\" $HIGHLIGHT >";
echo "<td>Issuing Amount:<td>Rs.&nbsp<INPUT NAME=\"issue_amount\" id=\"issue_amount\" TYPE=\"TEXT\" VALUE=\"\" size=\"10\" $HIGHLIGHT>";
echo "<tr><td>Repayment Period:<td><INPUT NAME=\"period\" id=\"period\" TYPE=\"TEXT\" VALUE=\"\" size=\"4\" $HIGHLIGHT> &nbsp;Months";
...
if($_REQUEST['op']=='v'){
	...
	echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"mt_loan_issue_ef.php?menu=$menu&op=i\">";
echo "<table bgcolor=\"#8FBC8F\" width=\"80%\" align=\"CENTER\" border=1>";
echo "<tr><th bgcolor=\"#FFD700\" colspan=\"4\"><font size=+2>MT Issue Form For verify[$account_no]</font>";
...
$sql="select repayment_mode,installment_mode from loan_repayment where loan_type like 'mt'";
//echo $sql_statement;
$res=dBConnect($sql);
...
//loan_ledger_hrd
		$sql_statement="INSERT INTO loan_ledger_hrd (loan_serial_no,loan_type, customer_id,account_no,fy,issue_date,repay_date,period,payment_term,int_due_rate,int_overdue_rate, applied_amount,status,staff_id,entry_time,gl_code,gl_status,loan_compt_status) VALUES ('$loan_sl_no','mt','$id','$account_no', '$fy1','$action_date','$repay_date',$period,'$payment_term',$due_int,$over_due_int,$applied_amount,'op', '$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$gl_code','d','$repayment_mode')";
//loan_security 
...
//gl_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks,operator_code, entry_time,cheque_no,cheque_dt) VALUES ('$t_id','mt','$action_date','$fy1','$remarks','$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$ch_no','$cheque_dt')";
//gl_ledger_dtl ===== Dr.
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$issue_amount,'Dr','mt issue')";
//gl_ledger_dtl ===== Cr.

		if($t_status=='c')
		{
			$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$issue_amount,'Cr','mt issue')";
		}
		if($t_status=='y')
		{
		...
		echo "<div class='failed'><h3><font color=\"white\">Failed to update database.</font></h3></div>";	
			...
			echo "<font size=+1><a href=\"../main/set_account.php?menu=mt&account_no=$account_no\">Click</a> here to go Statement</div>"; 
		}
