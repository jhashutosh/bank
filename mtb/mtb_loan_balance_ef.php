...
$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','mtb','$curr_date','$fy','$remarks', '$staff_id', CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
		
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$balance_p,'Dr','opening MTB')";
...
echo "<tr><th bgcolor=\"#006400\" colspan=\"4\"><font size=+2>MTB Issuing Form of [$account_no]</font>";
echo "<FORM NAME=\"f1\" method=\"POST\" action=\"mtb_loan_balance_ef.php?menu=$menu&op=i\">";
