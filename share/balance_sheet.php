...
//---------------------------------------------------------------------------------------------
echo "<hr>";
$sql_statement="SELECT master.gl_mas_code,initcap(b.gl_mas_desc) as gl_mas_desc,SUM(debit) as debit,sum(credit) as credit, sum(credit-debit) as bal FROM (
SELECT CASE 
...
UNION ALL
SELECT '12302' as gl_mas_code, 'Profit & Loss Balance As Per Current Year' as gl_mas_desc,SUM(debit) as debit,SUM(credit) as cedit,SUM(credit-debit) as bal FROM (SELECT a.gl_mas_code,SUM(debit) as debit,SUM(credit) as credit,bs_pl from mas_gl_tran a,gl_master_vw b WHERE (action_date BETWEEN '$f_start_dt' AND '$start_date') AND a.gl_mas_code=b.gl_mas_code group by a.gl_mas_code,bs_pl HAVING bs_pl='S' OR bs_pl='I' OR bs_pl='P' OR bs_pl='E') as foo order by gl_mas_code";
//echo $sql_statement;
