...
if($menu=='mt'){$i_page="../mtloan/mt_loan_issue_ef.php?menu=mt&account_no=";
...
if($flag==1){
	...
	$sql_statement="select status from loan_cal_int where loan_serial_no is not null";
	$result=dBConnect($sql_statement);
...
$sql_statement="select account_no,round(SUM(due_int)) as d,round(SUM(overdue_int)) as o,round(SUM(principal)) as p from loan_cal_int group by account_no ORDER BY cast(substr(account_no,position ('-' in account_no)+1) as integer)";

	}
	else
	{

$sql_statement="select account_no,round(SUM(due_int)) as d,round(SUM(overdue_int)) as o,round(SUM(principal)) as p from loan_cal_int WHERE status='d' OR status='o' group by account_no ORDER BY cast(substr(account_no,position ('-' in account_no)+1) as integer)";
...
echo "<tr><th bgcolor=\"Yello\" colspan=\"6\" align=center><font color=\"white\">Debtors Summary As On $end_date</font>";
$color="GREEN";
...
echo "<tr bgcolor=\"AQUA\"><th colspan=\"2\">Total:  ".$j." Account Found!!!!!<th align=\"right\">".amount2Rs($t_d_p)."<th align=\"right\">".amount2Rs($t_d_i)."<th align=\"right\">".amount2Rs($t_od_i)."<th align=\"right\">".amount2Rs($t_t);
}
}
else{
