...
echo "</table>";
echo "<form name=\"f1\" action=\"loan_od_list.php?menu=$menu\" method=\"POST\" onsubmit=\"return check();\">";
...
echo "<hr>";
if($flag==1){
$sql_statement="select account_no,round(SUM(due_int)) as d,round(SUM(overdue_int)) as o,round(SUM(principal)) as p from loan_cal_int WHERE crop_id='$crop_id' and (status='d' OR status='o') group by account_no ORDER BY account_no";
...
echo "<tr bgcolor=\"AQUA\"><th colspan=\"2\">Total:  ".($j-1)." Account Found!!!!!<th align=\"right\">".amount2Rs($t_d_p)."<th align=\"right\">".amount2Rs($t_d_i)."<th align=\"right\">".amount2Rs($t_od_i)."<th align=\"right\">".amount2Rs($t_t);
}
}
else{
