...
if($menu=='add'){
		$i_page="../add/add_loan_issue_ef.php?menu=add&account_no=";
		$r_page="../add/add_loan_repayment.php?menu=add&account_no=";
		$s_page="../add/add_statement.php?menu=add&op=i&account_no=";
	}	
...
$sql_statement="DELETE FROM loan_cal_int;SELECT loan_calculation('$menu','$end_date') as int";
//echo $sql_statement;
...
echo "<form name=\"f1\" action=\"member_list.php?menu=$menu\" method=\"POST\" onsubmit=\"return check();\">";
echo "<table align=center bgcolor=\"#90EE90\"><tr><td  align=\"\"><b>Detail List as on Dated (DD/MM/YYYY) :<td><input type=TEXT size=12 name=\"end_date\" id =\"end_date\" value=\"$end_date\" $HIGHLIGHT>";
...
echo "<hr>";
if($flag==1){
$sql_statement="select account_no,round(SUM(due_int)) as d,round(SUM(overdue_int)) as o,round(SUM(principal)) as p from loan_cal_int group by account_no ORDER BY cast(substring(account_no,4) as int) desc";
//echo $sql_statement; 
$result=dBConnect($sql_statement);
echo "<Table bgcolor=\"Black\" width=\"100%\" >";
echo "<tr><th bgcolor=\"Yello\" colspan=\"4\" align=center><font color=\"white\">Debtors Summary As On $end_date</font>";
$color="GREEN";
echo "<tr><th bgcolor=$color>Account No.</th>";
echo "<th bgcolor=$color>Name</th>";
echo "<th bgcolor=$color>Address</th>";
echo "<th bgcolor=$color>Gardian Name</th>";
...
echo "<tr bgcolor=\"AQUA\"><th colspan=\"4\">Total:  ".($j-1)." Account Found!!!!!";
}
else{
