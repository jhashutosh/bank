...
// Loan_Ledger_Hrd
	$sql_statement="INSERT INTO loan_ledger_hrd(loan_serial_no,loan_type,customer_id,account_no,issue_date,repay_date,int_due_rate, int_overdue_rate,status) VALUES ('$loan_sl_no','$menu','$id','$account_no','$action_date', '$repay_date',$due_int,$overdue_int,'op')";
//loan_security Updation 
	//$sql_statement.=";UPDATE loan_security SET status='l' WHERE loan_serial_no='$loan_sl_no'";

// loan_return_dtl
	$sql_statement=$sql_statement.";INSERT INTO loan_return_dtl(tran_id, loan_serial_no,account_no,action_date,r_total_amount,r_due_int,r_overdue_int,r_principal, b_due_int,b_overdue_int,b_principal,staff_id,entry_time)VALUES('$t_id','$loan_sl_no', '$account_no','$curr_date',($r_d_int+$r_od_int+$r_principal),$r_d_int,$r_od_int,$r_principal,$b_d_int,$b_od_int,$balance_p,'$staff_id',CAST(('2010-03-31 '||SUBSTR(now(),12,length(now()))) AS TIMESTAMP))";

// loan_issue_dtl
	$sql_statement=$sql_statement.";INSERT INTO loan_issue_dtl(tran_id, loan_serial_no,account_no, action_date,loan_amount,b_principal,staff_id,entry_time)VALUES('$t_id','$loan_sl_no','$account_no','$action_date',$issued_amount,$issued_amount,'$staff_id',(CAST(('2010-03-31 '||SUBSTR(now(),12,length(now()))) AS TIMESTAMP)-INTERVAL '1 days'))";
...
echo "<tr><th bgcolor=\"#006400\" colspan=\"4\"><font size=+2>MT-Loan Opening Issuing Form of [$account_no]</font>";
...
if(empty($_REQUEST['op'])&&$flag==1&&!isOpenLoan($account_no)){
?>
