...
//added existing
$Repay_Period=$_REQUEST['Repay_Period'];
...
//added existing
	$sql_statement="INSERT INTO loan_ledger_hrd(loan_serial_no,loan_type,customer_id,account_no,issue_date,repay_date,period,int_due_rate, int_overdue_rate,status,gl_code,gl_status) VALUES ('$loan_sl_no','$menu','$id','$account_no','$action_date', '$repay_date','$Repay_Period',$due_int,$overdue_int,'op','$gl_code_loan','$gl_status')";
...
//general ledger posting
	//gl_ledger_hrd
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','hou','$curr_date','$fy','$remarks', '$staff_id', CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
//gl_ledger_dtl		
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code_loan',$balance_p,'Dr','opening HOUSE')";
...
echo "<tr><th bgcolor=\"#006400\" colspan=\"6\"><font size=+2>HOUSE LOAN EXSISTINF ENTRY FROM [$account_no]</font>";
echo "<FORM NAME=\"f1\" method=\"POST\" action=\"house_loan_balance_ef.php?menu=$menu&op=i\">";
...
if(empty($loan_sl_no)){
echo "<INPUT TYPE=\"button\" VALUE=\"  Add  \" onClick=\"openChild('addDocument.php?menu=pl&ls=o','win2')\">";
}
else{
...
//added existing
echo "<td>Repay Period:<td><INPUT NAME=\"Repay_Period\" TYPE=\"TEXT\" VALUE=\"\" $HIGHLIGHT size=\"4\">";
...
//added existing
 frmvalidator.addValidation("Repay_Period","req","Please enter Repay_Period");
