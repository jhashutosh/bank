...
/***********************************FIRST FORM *********************************************/
	if(empty($op)){
		...
		if($flag){
			...
			echo "<tr><td align=\"left\">Due Interest Rate:<td>Rs.&nbsp;<input type=\"TEXT\" name=\"d_r\" size=\"5\" value=\"10\" $HIGHLIGHT>&nbsp;%";
			echo "<td align=\"left\">Over DueInterest Rate:<td>Rs.&nbsp;<input type=\"TEXT\" name=\"o_d_r\" size=\"5\" value=\"10\" $HIGHLIGHT>&nbsp;%";
...
echo "<tr><td colspan=\"6\" align=\"right\" bgcolor=ORANGE><input type=\"SUBMIT\" name=\"SUBMIT_BUTTON\" value=\"Enter\">";
echo "</table>";
}
...
//loan_ledger_hrd
		$sql_statement=$sql_statement.";INSERT INTO loan_ledger_hrd(loan_serial_no,customer_id, account_no,fy,issue_date,int_due_rate,int_overdue_rate, applied_amount,repay_date,period, max_limit,staff_id,status,entry_time,gl_code,gl_status)VALUES('$loan_sl_no','$c_id','$account_no','$fy','$action_date',$d_r,$o_d_r,$a_amount,'$period','$repay_date',$loan_limit,'$staff_id','op',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP),'$type','d')";
//loan_issue_dtl
