...
echo "<i>information for kcc loan system</i><hr>";
getLoanInt('kcc',date('d/m/Y'),$due,$over,$crop_id);
$gl_code=getGlCode4mCustomerAccount($account_no,$action_date);
$sql_statement="INSERT INTO loan_ledger_hrd(loan_serial_no,loan_type,customer_id,account_no,fy,crop_id,max_limit,int_due_rate, int_overdue_rate,gl_code,gl_status) VALUES ('$loan_sl_no','kcc','$id','$account_no','$fy1','$crop_id',$max_limit,$due,$over,$gl_code,'d')";
...
//echo $sql_statement;
  ...
  if(pg_affected_rows($result)<1){
	...
	echo "<font size=+2 color=green>Successfully Inserted data into database.&nbsp;<input type=button onclick=\"myRefresh('kcc_loan_issue.php?menu=kcc')\" value=\"Return\"></font>";
