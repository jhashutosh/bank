...
if($_REQUEST['op']=='i'){
	...
	$t_id=getTranId();
	$sql_statement="INSERT INTO loan_ledger_hrd(loan_serial_no,loan_type,customer_id,account_no,issue_date,repay_date,crop_id,max_limit, int_due_rate, int_overdue_rate,status,gl_code,gl_status) VALUES ('$loan_sl_no','kcc','$id','$account_no','$action_date','$repay_date','$crop_id',$issued_amount,$due_int,$overdue_int,'op','$gl_code_loan','$gl_status')";
	...
	$sql_statement=$sql_statement.";INSERT INTO loan_return_dtl(tran_id, loan_serial_no,account_no,action_date,r_total_amount,r_due_int,r_overdue_int,r_principal, b_due_int,b_overdue_int,b_principal,staff_id,entry_time)VALUES('$t_id','$loan_sl_no', '$account_no','$curr_date',($r_d_int+$r_od_int+$r_principal),$r_d_int,$r_od_int,$r_principal,$b_d_int,$b_od_int,$balance_p,'$staff_id',CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
       $sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','kcc','$curr_date','$fy','$remarks', '$staff_id', CAST('$curr_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
...
echo "<td bgcolor=\"#90EE90\">Over Due Interest Rate:<td bgcolor=\"#90EE90\">Rs.&nbsp;<INPUT NAME=\"od_int_r\" TYPE=\"TEXT\" VALUE=\"11\" $HIGHLIGHT size=\"3\" maxlength=\"2\">&nbsp;%";
