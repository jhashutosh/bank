...
$sql_statement="SELECT getLoanInterest('$account_no','$action_date','sao') as int";
//echo $sql_statement;
...
if(pg_NumRows($result)>0){
	$color="GREEN";
	echo "<tr><th bgcolor=YELLOW colspan=\"9\">SAO Details of [$account_no] as on ".$action_date."</th>";
	echo "<tr>";
	...
	echo "<td align=Center bgcolor=$color><a href=\"sao_loan_repayment.php?menu=$menu&ln_sl=$loan_sl_no&op=r&action_date=$action_date\"><font color=$fcolor>Repay</td>";

	}
}
else{
 echo "<h1><center>Your dont have any SAO Loan!!!!!!!!!!!</h1></center>";
    }
}
//==================================FOR REPAY FORM=========================================
if($op=='r'){
...
echo "<tr><th bgcolor=\"#808000\" colspan=\"4\"><font size=+2>SAO Repayment Form of [$account_no]</font>";
echo "<FORM NAME=\"pForm\" method=\"POST\" action=\"sao_loan_repayment.php?menu=$menu&op=i&ln_sl=$ln_sl\" onSubmit=\"return checksaoReturn(this.form);\">";
...
if(empty($fy)){
	...
	//For GL ENTRY-------------------------------------
	$sql_statement.=";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','sao','$action_date','$fy','$remarks','$staff_id',CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
	$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',$amount,'Dr','sao repay')";
	if($r_i_od>0){
