...
if($_REQUEST['op']=='i'){
	...
	if(empty($fy)){
		...
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_hrd(tran_id,type, action_date,fy,remarks, operator_code, entry_time) VALUES ('$t_id','sao','$action_date','$fy','$remarks', '$staff_id', CAST('$action_date'||SUBSTR(CAST(now() AS VARCHAR),11,LENGTH(CAST(NOW() AS VARCHAR))) AS TIMESTAMP))";
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,account_no,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','$account_no','$gl_code',$amount,'Dr','sao issue')";
		...
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars) VALUES('$t_id','28101',($amount-$c_insurance-$s_insurance),'Cr','sao issue')";
		}
		else{
		$gl_code=getGlCode4mCustomerAccount($_REQUEST['account_sb'],$action_date);
		$sql_statement=$sql_statement.";INSERT INTO gl_ledger_dtl(tran_id,gl_mas_code,amount,dr_cr, particulars,account_no) VALUES('$t_id','$gl_code',($amount-$c_insurance-$s_insurance),'Cr','sao issue',trim('".$_REQUEST['account_sb']."'))";
		}
		//echo $sql_statement;
		...
		echo "<br><h5><font color=\"RED\">Failed to insert data into database.</font></h5>";
			...
			echo "</font></pre>";
			echo "<font size=+1><a href=\"../main/set_account.php?menu=sao&account_no=$account_no\">Click</a> here to go Statement"; 
...
echo "<tr><th bgcolor=\"#9400D3\" colspan=\"4\"><font size=+2>SAO Issuing Varify Form of [$account_no]</font>";
echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"sao_loan_issue.php?menu=$menu&op=i\">";
...
echo "<td align=\"RIGHT\" bgcolor=\"#90EE90\"><input type=\"SUBMIT\" VALUE=\"   Final  \" >";
		...
		echo "<h3><b>You have Taken SAO Loan is: Rs. $issued_amount /=</h3>";
		echo "<h3><b>You Demand for SAO Loan again is: Rs. $amount /=</h3>";
...
echo "<table bgcolor=\"BLACK\" width=\"80%\" align=\"CENTER\">";
echo "<tr><th bgcolor=\"#9400D3\" colspan=\"4\"><font size=+2>SAO Issuing Form of [$account_no]</font>";
if($cflag==1){
echo "<FORM NAME=\"Form1\" method=\"POST\" action=\"sao_loan_issue.php?menu=$menu\">";
echo "<tr><td bgcolor=\"#90EE90\"> Select Crop:<td bgcolor=\"#90EE90\">";
cropSelection('crop_id');
echo "<td bgcolor=\"#90EE90\" colspan=\"2\">";
echo "</form>";
}
echo "<FORM NAME=\"parentForm\" method=\"POST\" action=\"sao_loan_issue.php?menu=$menu&op=v\" onSubmit=\"return checkSaoIssue(this.form);\">";
if(!empty($crop_id)){
...
echo "<td bgcolor=\"#90EE90\">Add Credit Limit:<td bgcolor=\"#90EE90\"><INPUT TYPE=\"button\" VALUE=\"Add Land\" onClick=\"openChild('addLand.php?menu=sao','win2')\">";
}
if($fflag==1){
echo "<tr><td bgcolor=\"#90EE90\">Due Interest Rate:<td bgcolor=\"#90EE90\">Rs.&nbsp;<INPUT NAME=\"d_int_r\" TYPE=\"TEXT\" VALUE=\"$due_int\" $HIGHLIGHT size=6 READONLY>&nbsp;%";
echo "<td bgcolor=\"#90EE90\">Over Due Interest Rate:<td bgcolor=\"#90EE90\">Rs.&nbsp;<INPUT NAME=\"od_int_r\" TYPE=\"TEXT\" VALUE=\"$overdue_int\" $HIGHLIGHT size=\"6\" readonly>&nbsp;%";
